...
resolve_node_runtime() {
  ...
  if [ -n "${BAIYING_ELECTRON_PATH:-}" ] && [ -x "${BAIYING_ELECTRON_PATH}" ]; then
    NODE_CMD="$BAIYING_ELECTRON_PATH"
    NODE_ARGS=()
...
ensure_npm_available() {
  ...
  echo "  Please reinstall the web-search skill runtime from BaiYing."
  return 1
}
...
if ! resolve_node_runtime; then
  ...
  echo "  Please install Node.js, or run from BaiYing so scripts can use Electron runtime."
  exit 1
fi
