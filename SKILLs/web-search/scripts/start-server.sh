...
resolve_node_runtime() {
  ...
  if [ -n "${baiyingAI_ELECTRON_PATH:-}" ] && [ -x "${baiyingAI_ELECTRON_PATH}" ]; then
    NODE_CMD="$baiyingAI_ELECTRON_PATH"
    NODE_ARGS=()
...
ensure_npm_available() {
  ...
  echo "  Please reinstall the web-search skill runtime from baiyingAI."
  return 1
}
...
if ! resolve_node_runtime; then
  ...
  echo "  Please install Node.js, or run from baiyingAI so scripts can use Electron runtime."
  exit 1
fi
