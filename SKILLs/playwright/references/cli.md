...
```bash
export SKILLS_ROOT="${BAIYING_SKILLS_ROOT:-${SKILLS_ROOT:-$HOME/Library/Application Support/BaiYing/SKILLs}}"
export PWCLI="$SKILLS_ROOT/playwright/scripts/playwright_cli.sh"
"$PWCLI" --help
```

Installed skills resolve from `$BAIYING_SKILLS_ROOT` / `$SKILLS_ROOT` (production default: app `userData/SKILLs`, macOS usually `~/Library/Application Support/BaiYing/SKILLs`).
