...
2. 确认目标数据库路径。默认桌面版用户数据路径通常是：  
   ...
   ```bash
   ~/Library/Application\ Support/BaiYing/baiying.sqlite
   ```
...
npm run test:sqlite-backup:seed -- \
  --db '/Users/jj.deng/Library/Application Support/BaiYing/baiying.sqlite' \
  --sessions 10 \
...
```bash
BAIYING_SQLITE_BACKUP_ALWAYS_ON_STARTUP=1
```
...
```bash
BAIYING_SQLITE_BACKUP_ALWAYS_ON_STARTUP=1 npm run electron:dev
```
...
- `[SqliteBackup] Starting periodic backup to baiying-latest.sqlite`
...
```bash
ls -lh ~/Library/Application\ Support/BaiYing/backups/sqlite/snapshots/
```
...
```bash
printf 'not-a-sqlite-db' > ~/Library/Application\ Support/BaiYing/baiying.sqlite
```
...
4. 使用 `BAIYING_SQLITE_BACKUP_ALWAYS_ON_STARTUP=1` 启动应用。  
   Launch the app with `BAIYING_SQLITE_BACKUP_ALWAYS_ON_STARTUP=1`.
5. 记录备份开始、进度、完成日志及耗时。  
