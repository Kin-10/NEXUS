...
const DEFAULTS = {
  db: path.resolve(process.cwd(), 'baiying.sqlite'),
  sessions: 20,
...
Examples:
  node tests/sqlite-backup/generate-large-db.cjs --db ~/tmp/baiying.sqlite
  node tests/sqlite-backup/generate-large-db.cjs --db ./tmp/baiying.sqlite --sessions 50 --messages-per-session 10000 --payload-kb 16
`);
}
...
function main() {
  ...
  const tempSqlPath = path.join(
    os.tmpdir(),
    `baiying-sqlite-backup-seed-${process.pid}-${Date.now()}.sql`,
  );
