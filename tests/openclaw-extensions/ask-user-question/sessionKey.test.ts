...
describe('ask-user-question session key gating', () => {
  test('allows desktop sessions across agents', () => {
    expect(isAskUserQuestionCandidateSessionKey('agent:main:baiying:session-1')).toBe(true);
    expect(isAskUserQuestionCandidateSessionKey('agent:qa-reviewer:baiying:session-2')).toBe(true);
  });
  ...
  test('allows legacy desktop sessions', () => {
    expect(isAskUserQuestionCandidateSessionKey('baiying:session-3')).toBe(true);
  });
  ...
  test('rejects channel and malformed session keys', () => {
    ...
    expect(isAskUserQuestionCandidateSessionKey('agent::baiying:session-4')).toBe(false);
    expect(isAskUserQuestionCandidateSessionKey('agent:qa-reviewer:baiying:')).toBe(false);
