...
test('parseManagedSessionKey handles raw local session keys', () => {
  assert.deepEqual(parseManagedSessionKey('baiying:abc-123'), {
    agentId: null,
    sessionId: 'abc-123',
  });
});
...
test('parseManagedSessionKey handles canonical local session keys', () => {
  assert.deepEqual(parseManagedSessionKey('agent:main:baiying:abc-123'), {
    agentId: 'main',
    sessionId: 'abc-123',
  });
});
...
test('buildManagedSessionKey emits canonical local session keys', () => {
  assert.equal(
    ...
    `agent:${DEFAULT_MANAGED_AGENT_ID}:baiying:abc-123`,
  );
  assert.equal(
    buildManagedSessionKey('abc-123', 'secondary'),
    'agent:secondary:baiying:abc-123',
  );
});

test('parseChannelSessionKey ignores managed local session keys', () => {
  assert.equal(parseChannelSessionKey('baiying:abc-123'), null);
  assert.equal(parseChannelSessionKey('agent:main:baiying:abc-123'), null);
});

test('channel sync does not treat managed local session keys as channel sessions', () => {
  const sync = createSync();

  assert.equal(isManagedSessionKey('agent:main:baiying:abc-123'), true);
  assert.equal(sync.isChannelSessionKey('agent:main:baiying:abc-123'), false);
  assert.equal(sync.resolveOrCreateSession('agent:main:baiying:abc-123'), null);
