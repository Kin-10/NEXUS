...
describe('ensure-openclaw-plugins', () => {
  ...
  test('detects local path specs', () => {
    expect(isLocalPathSpec('/tmp/example-openclaw-plugin')).toBe(true);
    expect(isLocalPathSpec('./plugins/example-openclaw-plugin')).toBe(true);
    expect(isLocalPathSpec('@scope/openclaw-plugin')).toBe(false);
  });

  test('detects git specs from GitHub', () => {
    expect(isGitSpec('git+https://github.com/example/openclaw-plugin.git')).toBe(true);
    expect(isGitSpec('https://github.com/example/openclaw-plugin.git')).toBe(true);
    expect(isGitSpec('github:example/openclaw-plugin')).toBe(true);
  ...
  test('appends version as git ref when the spec has no hash', () => {
    expect(resolveGitPackSpec(
      'git+https://github.com/example/openclaw-plugin.git',
      '1.0.3',
    )).toBe('git+https://github.com/example/openclaw-plugin.git#1.0.3');

    expect(resolveGitPackSpec(
      'git+https://github.com/example/openclaw-plugin.git#main',
      '1.0.3',
    )).toBe('git+https://github.com/example/openclaw-plugin.git#main');
  });

  test('resolves git sources to packed installs', () => {
    expect(resolvePluginInstallSource({
      id: 'example-openclaw-plugin',
      npm: 'git+https://github.com/example/openclaw-plugin.git',
      version: '1.0.3',
    })).toEqual({
      kind: 'git',
      gitSpec: 'git+https://github.com/example/openclaw-plugin.git#1.0.3',
      pinnedDisplaySpec: 'git+https://github.com/example/openclaw-plugin.git#1.0.3',
    });
  });
  ...
  test('parses git specs into clone url and ref', () => {
    expect(parseGitSpec(
      ...
      cloneUrl: 'https://github.com/example/openclaw-plugin.git',
      ref: '1.1.0',
    });

    expect(parseGitSpec(
      'github:example/openclaw-plugin#main',
      '1.1.0',
    )).toEqual({
      cloneUrl: 'https://github.com/example/openclaw-plugin.git',
      ref: 'main',
  ...
  test('preserves existing registry and local path behavior', () => {
    expect(resolvePluginInstallSource({
      id: 'private-registry-plugin',
      npm: 'private-registry-plugin',
      version: '2.0.7',
      registry: 'https://npm.example.test',
    })).toEqual({
      kind: 'packed',
      packSpec: 'private-registry-plugin@2.0.7',
      pinnedDisplaySpec: 'private-registry-plugin@2.0.7',
      registry: 'https://npm.example.test',
    });
