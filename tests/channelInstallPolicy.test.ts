  ...
  spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        KEYFROM: 'dictbind',
        BAIYING_CHANNEL_BUILD: '1',
        BAIYING_SILENT_ON_DOUBLE_CLICK: silentOnDoubleClick ? '1' : '0',
        BAIYING_WEB_INSTALLER: '1',
        BAIYING_WEB_PKG_URL: 'https://cdn.example.test/BaiYing.nsis.7z',
      },
      encoding: 'utf8',
    },
  )
  ...
  spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...
        KEYFROM: 'dictbind',
        BAIYING_CHANNEL_BUILD: '1',
        BAIYING_SILENT_ON_DOUBLE_CLICK: '1',
        BAIYING_WEB_INSTALLER: '1',
        BAIYING_WEB_PKG_URL: '',
        BAIYING_WEB_PKG_BASE_URL: 'https://cdn.example.test/releases',
      },
      encoding: 'utf8',
    },
  )
...
describe('channel installer build flags', () => {
  ...
  test('does not leak inherited build env into a channel dry-run', () => {
    ...
    expect(inherited.stderr).toContain('ignoring inherited BAIYING_CHANNEL_BUILD=1');
    expect(inherited.stderr).toContain('ignoring inherited BAIYING_SILENT_ON_DOUBLE_CLICK=1');
  });
});
...
describe('web installer build flags', () => {
  ...
  test('carries the silent flag through the two-pass web build instructions', () => {
    ...
    expect(silent.stdout).toMatch(
      /next: upload release[\\/]nsis-web[\\/]baiying-[^\s]+-x64\.nsis\.7z/,
    );
  });
  ...
  test('uses the prepackaged app for the stub-only pass', () => {
    const stubOnly = runWebDryRun([
      ...
      '--silent',
      '--pkg-url',
      'https://cdn.example.test/baiying.nsis.7z',
    ]);
  ...
  test('does not leak inherited silent flags into a web build', () => {
    ...
    expect(inherited.stderr).toContain(
      'ignoring inherited BAIYING_SILENT_ON_DOUBLE_CLICK=1',
    );
  });
  ...
  test('marks only double-click-silent web artifacts in their filename', () => {
    ...
    expect(plain.stdout).toContain(
      'artifact=BaiYing-WebSetup-${arch}-${version}-dictbind.${ext}',
    );
    expect(silent.status).toBe(0);
    expect(silent.stdout).toContain(
      'artifact=BaiYing-WebSetup-${arch}-${version}-dictbind-silent.${ext}',
    );
  });
  ...
  test('uses the actual lowercase electron-builder package name for base URLs', () => {
    ...
    expect(probe.stdout).toMatch(
      /packageUrl=https:\/\/cdn\.example\.test\/releases\/dictbind\/baiying-[^/\s]+-x64\.nsis\.7z/,
    );
  });
});
