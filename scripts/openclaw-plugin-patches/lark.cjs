...
// Lightweight setup entry for deferred loading (patched by BaiYing).
...
function patchFilenameEncoding(larkPluginDir, log) {
  ...
  const replacement = `fileName = decodeURIComponent(match[1].trim());
                // Patched by BaiYing: fix Latin-1 garbled UTF-8 filenames from Feishu API
  ...
  if (fnIdx !== -1) {
    const helperFn = `// Patched by BaiYing: detect and fix Latin-1 garbled UTF-8 filenames.
