 ...
 * 通过 BaiYing.exe (ELECTRON_RUN_AS_NODE=1 模式) 执行。
 *
 * 用法: BaiYing.exe <本脚本路径> <tarPath> <destDir>
 *
 * 效果:
...
if (!tarPath || !destDir) {
  console.error('[unpack-cfmind] Usage: BaiYing.exe unpack-cfmind.cjs <tarPath> <destDir>');
  process.exit(1);
}
