...
;
; 1. BaiYing.exe -- the main app AND the OpenClaw gateway (ELECTRON_RUN_AS_NODE)
; 2. node.exe whose binary lives inside the BaiYing install tree
...
!macro stopBaiYingProcesses
  ...
  Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
    ...
    for ($$i = 0; $$i -lt 30; $$i++) {\
      ...
      $$procs += Get-Process node -ErrorAction SilentlyContinue | Where-Object { $$_.Path -like \"*BaiYing*\" };\
  ...
  StopBaiYingProcessesDone:
  StrCpy $R2 "helper-not-found"

  StopBaiYingProcessesLog:
  ; Clearing variables that were never set (helper-not-found path) is a no-op.
  System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_STOP_ROOT", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_STOP_SELF_PID", t "")i'
...
!macro customCheckAppRunning
  ...
  !ifndef BUILD_UNINSTALLER
    ...
    ; Copy non-bundled skills to %APPDATA%\BaiYing\skills-backup\ so they are
    ...
    Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
      ...
      $$oldVer    = $$env:BAIYING_OLD_VERSION;\
    ...
    Pop $1
    StrCpy $R2 $0
    System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_SKILL_SOURCE", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_SKILL_BACKUP_ROOT", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_INSTALL_ATTEMPT_ID", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_OLD_VERSION", t "")i'
    System::Call 'kernel32::GetTickCount()i .r6'
    IntOp $5 $6 - $7
...
!ifndef BUILD_UNINSTALLER
  ...
  ; remove BaiYing exclusions at the end of their --updated flow.
...
!macro customBeforeRegistryAddInstallInfo
  ...
  ; Log file: %APPDATA%\BaiYing\install-timing.log
  ...
  Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
    ...
    function Write-LobsterWatchdogMarker {\
      ...
      try {\
        ...
        Write-Output (\"BAIYING_WATCHDOG_MARKER_WRITE_FAILED:\" + $$value)\
      }\
    };\
    try {\
    ...
    if ($$p.WaitForExit(600000)) {\
      ...
      if ($$p.ExitCode -eq $$null) {\
        $$sentinelOk = $$false;\
        try {\
          $$sentinel = Join-Path $$env:BAIYING_EXTRACTOR_DESTINATION \".unpack-cfmind-ok\";\
  ...
  System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_EXTRACTOR_EXE", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_EXTRACTOR_SCRIPT", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_EXTRACTOR_ARCHIVE", t "")i'
  System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_EXTRACTOR_DESTINATION", t "")i'
  ...
  MessageBox MB_OK|MB_ICONEXCLAMATION "The BaiYing installation stopped because resource extraction completed without all required runtime resources ($R5). The installer will not commit a partial application. Details: $APPDATA\BaiYing\install-timing.log" /SD IDOK
  ...
  SkillRestoreAttemptBackupReady:
    ...
    Push '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
      ...
      $$attempt   = $$env:BAIYING_INSTALL_ATTEMPT_ID;\
      $$root      = $$env:BAIYING_SKILL_BACKUP_ROOT;\
      $$source    = $$env:BAIYING_SKILL_SOURCE;\
      $$backup    = Join-Path $$root $$attempt;\
      $$newSkills = $$env:BAIYING_SKILL_DESTINATION;\
      try {\
        if ([string]::IsNullOrWhiteSpace($$attempt)) { throw \"attempt id missing\" };\
    ...
    System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_SKILL_SOURCE", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_SKILL_BACKUP_ROOT", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_SKILL_DESTINATION", t "")i'
    System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_INSTALL_ATTEMPT_ID", t "")i'
    Goto SkillRestoreCommandDone
...
!macro customInstall
  ...
  ${If} $lobsterOldInstallRenameStatus == "committed"
    ...
    System::Call 'Kernel32::SetEnvironmentVariable(t "BAIYING_OLD_CLEANUP_PATH", t "")i'
    FileOpen $2 "$APPDATA\BaiYing\install-timing.log" a
    FileSeek $2 0 END
    !insertmacro GetTimestamp $8
