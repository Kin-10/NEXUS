...
// reproducible from any terminal state (see BAIYING_WEB_* handling below).
...
function webPackageFileName(version) {
  return `baiying-${version}-x64.nsis.7z`;
}

function webSetupFileName(version, keyfrom, silentOnDoubleClick) {
  return `BaiYing-WebSetup-x64-${version}-${keyfrom}${silentOnDoubleClick ? '-silent' : ''}.exe`;
}
...
Modes:
  ...
  --pkg-base-url  one-pass build; the installer downloads <dir>/<keyfrom>/baiying-<version>-x64.nsis.7z
