...
/**
 * Apply version-specific BaiYing patches to the openclaw source tree.
 *
 ...
 * otherwise the source defaults to ../openclaw relative to the BaiYing
 * project root.
 *
  ...
  'openclaw-baiying-model-compat-api.patch': [
    {
      ...
      snippets: [
        'BAIYING_MODEL_COMPAT_API = "baiying-model-compat"',
...
// This removes stale patches left by a different BaiYing branch that may have
...
for (const patchFile of patchFiles) {
  ...
  if (needsNormalize) {
    patchPath = path.join(os.tmpdir(), `baiying-patch-${patchFile}`);
