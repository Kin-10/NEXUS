...
// changes unless BAIYING_WEB_INSTALLER=1 is set explicitly.
  ...
  if (!raw) {
    throw new Error(
      `[WebInstaller] either ${WEB_PKG_URL_ENV} (exact package URL from object storage) or ` +
        `${WEB_PKG_BASE_URL_ENV} (CDN base directory, e.g. https://cdn.example.com/baiying/win) ` +
...
config.dmg = {
  ...(config.dmg || {}),
  artifactName: `BaiYing-darwin-\${arch}-\${version}-${keyfrom}.\${ext}`,
};

config.nsis = {
  ...(config.nsis || {}),
  artifactName: `BaiYing-Setup-\${arch}-\${version}-${keyfrom}${silentOnDoubleClick ? '-silent' : ''}.\${ext}`,
};
...
if (isWebInstallerEnabled()) {
  ...
  config.nsisWeb = {
    ...
    artifactName: `BaiYing-WebSetup-\${arch}-\${version}-${keyfrom}${silentOnDoubleClick ? '-silent' : ''}.\${ext}`,
  };
