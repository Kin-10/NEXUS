# 审计：BaiYing Provider 配置 vs OpenClaw 官方文档
...
| BaiYing ProviderName | 当前 OpenClawProviderId | OpenClaw 官方 Provider ID | 状态 |
|---|---|---|---|
...
**现状**：BaiYing 使用 `OpenClawProviderId.Zhipu = 'zhipu'`
...
**实际影响**：由于 BaiYing 使用 `models.mode: 'replace'` 注入完整 provider 配置（baseUrl + api + apiKey + models），`zhipu` 会被 OpenClaw 当作**自定义 provider** 处理，基本推理功能不受影响——请求会正确发送到我们指定的 `baseUrl`。
...
**现状**：BaiYing 将 volcengine 的 codingPlan 模式直接用 `volcengine` provider ID 处理，没有单独的 codingPlan 路由。
...
**影响**：当用户在 BaiYing 中启用 volcengine 的 codingPlan 模式时，生成的配置使用 `volcengine` 作为 provider ID，而 OpenClaw 期望的是 `volcengine-plan`。这可能导致 coding 模型无法被正确路由。
...
**现状**：BaiYing 将 deepseek 映射到 `OpenClawProviderId.DeepSeek = 'deepseek'`
...
| 位置 | 硬编码值 | 应替换为 |
|---|---|---|
| 第 762 行 | `'baiying-server'` | `ProviderName.BaiyingServer` |
| 第 770 行 | `'baiying-server'` | `ProviderName.BaiyingServer` |
| 第 778 行 | `'openai-completions'` | `OpenClawApi.OpenAICompletions` (cast) |
| 第 785 行 | `'baiying-server'` | `OpenClawProviderId.BaiyingServer` |

### 问题 7：缺少 OpenClaw 支持的 Provider

OpenClaw 官方文档列出了以下 BaiYing 目前不支持但 OpenClaw 已支持的 provider：
