...
`buildProviderSelection`（`src/main/libs/openclawConfigSync.ts`，搜索 `const buildProviderSelection` 定位）负责将 BaiYing 的 provider 配置映射为 OpenClaw 的 `OpenClawProviderSelection` 结构。当前存在三个核心问题：
...
| Provider | 处理方式 | 状态 |
|----------|----------|------|
| `baiying-server` | 专用分支（token proxy） | ✅ 已覆盖 |
| `moonshot` + codingPlan | 专用分支（kimi-coding） | ✅ 已覆盖 |
| `moonshot` | 专用分支 | ✅ 已覆盖 |
| `gemini` | 专用分支（google-generative-ai） | ✅ 已覆盖 |
| `anthropic` | 落入 default `baiying` | ⚠️ providerId 错误 |
| `openai` | 落入 default `baiying` | ⚠️ providerId 错误 |
| `deepseek` | 落入 default `baiying` | ⚠️ providerId 错误 |
| `qwen` | 落入 default `baiying` | ⚠️ 无 codingPlan URL 切换 |
| `zhipu` | 落入 default `baiying` | ⚠️ 无 codingPlan URL 切换 |
| `volcengine` | 落入 default `baiying` | ⚠️ 无 codingPlan URL 切换 |
| `minimax` | 落入 default `baiying` | ⚠️ providerId 错误 |
| `hzbzhiyun` | 落入 default `baiying` | ⚠️ providerId 错误 |
| `stepfun` | 落入 default `baiying` | ⚠️ providerId 错误 |
| `xiaomi` | 落入 default `baiying` | ⚠️ providerId 错误 |
| `openrouter` | 落入 default `baiying` | ⚠️ providerId 错误 |
| `ollama` | 落入 default `baiying` | ⚠️ providerId 错误 |
| `custom` | 落入 default `baiying` | ✅ 合理兜底 |
...
| 类别 | 散落的字符串 | 出现位置 |
|------|-------------|---------|
| Provider Name | `'baiying-server'`, `'moonshot'`, `'gemini'` 等 | `buildProviderSelection` 的 if 条件、`claudeSettings.ts` 的 `getEffectiveProviderApiFormat`、renderer `config.ts` 的 key |
| OpenClaw Provider ID | `'baiying'`, `'kimi-coding'`, `'google'` 等 | `buildProviderSelection` 返回值、`syncManagedSessionStore` 迁移逻辑 |
...
```typescript
// ─── Provider Name ──────────────────────────────────────────────────────
// providerName 用于标识 BaiYing 内部的 provider（对应 config key）
...
type ProviderDescriptor = {
   ...
   * Most providers use env var placeholders; baiying-server may use 'proxy-managed'.
   * Default: returns '${baiying_APIKEY_<PROVIDER>}'.
   */
...
const DEFAULT_DESCRIPTOR: ProviderDescriptor = {
  providerId: '', // 会被运行时动态设置为 providerName 或 'baiying'
  resolveApi: ({ apiType }) => mapApiTypeToOpenClawApi(apiType),
  normalizeBaseUrl: stripChatCompletionsSuffix,
};
```
...
const resolveDescriptor = (
  ...
  // or 'baiying' for empty names
  return {
    ...DEFAULT_DESCRIPTOR,
    providerId: providerName || 'baiying',
  };
};
...
| providerId 正确性 | 11 个 provider 被错误映射为 'baiying' | 所有 provider 使用正确的 ID |
...
2. 运行 `npm test -- openclawConfigSync`（Vitest），验证以下行为保持不变（这些测试在 Step 4 编写，但 Step 2 完成后即可运行）：
   - 场景 5: `buildProviderSelection({ providerName: 'anthropic', ... })` → `providerId === 'anthropic'`（不再是 `'baiying'`）
   - 场景 9: `buildProviderSelection({ providerName: 'ollama', ... })` → `providerId === 'ollama'`
   - 场景 11: `buildProviderSelection({ providerName: '', ... })` → `providerId === 'baiying'`（兜底行为保持）

### Step 3: 保留 `baiying-server` 的日志

原函数中 `baiying-server` 分支有 `console.log` 输出调试信息（搜索 `buildProviderSelection baiying-server` 定位），需在 descriptor 的 `normalizeBaseUrl` 或核心函数中保留。

**QA**（通过 Step 4 的测试验证，运行 `npm test -- openclawConfigSync`）:
1. 测试用例 `'baiying-server logs proxy info'`：使用 `vi.spyOn(console, 'log')` 断言当 `providerName === 'baiying-server'` 时，`console.log` 被调用且参数包含 `'[OpenClawConfigSync]'` 标签。
...
```typescript
// 场景 1: baiying-server（token proxy 模式）
// 输入: providerName='baiying-server', baseURL='http://example.com/v1/chat/completions', apiType='openai'
// 断言:
//   providerId === 'baiying-server'
...
// 断言:
//   providerId === 'my-custom-provider' (使用 providerName，不再是 'baiying')
//   api === 'openai-completions'
...
| 风险 | 影响 | 缓解措施 |
|------|------|----------|
| providerId 变更导致 session 迁移失效 | 已有 session 的 `modelProvider` 字段与新 providerId 不匹配 | `syncManagedSessionStore` 仅迁移 `baiying` → 新 ID 的 session，不影响已正确映射的 session |
| Registry 遗漏 provider | 落入 DEFAULT_DESCRIPTOR | DEFAULT_DESCRIPTOR 使用 providerName 作为 providerId，比原来的 'baiying' 更准确 |
