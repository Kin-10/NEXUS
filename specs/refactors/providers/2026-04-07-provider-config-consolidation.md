...
const providerKeys = [
  ...Object.values(ProviderName).filter(id => id !== ProviderName.Custom && id !== ProviderName.BaiyingServer),
  ...CUSTOM_PROVIDER_KEYS,
] as const;
...
> **注：** `ProviderName.Custom`（`'custom'`）是遗留占位符，实际使用 `custom_0`…`custom_9`，需过滤掉。`BaiyingServer` 是后端内部 provider，不在 UI 中展示，同样过滤。
