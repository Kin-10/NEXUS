...
部分用户在 Cowork 中选择 `qwen3.6-plus` / `qwen3.6-plus-hzbInner` 后发送图片，LLM 回复“看不到图片”。从日志看，图片并不是上传失败，而是在 OpenClaw gateway 进入 LLM 前被提前丢弃。

需要满足以下要求：

1. `baiying-server/qwen3.6-plus-hzbInner` 支持图片输入时，OpenClaw 配置必须写出 `input: ['text', 'image']`
...
7. qwen3.6 Plus 任务已经完成后，迟到的 OpenClaw lifecycle 事件不能把 BaiYing 会话重新置为 `running`

## 核心结论

**这不是 qwen3.6 Plus 模型本身不支持图片，而是 BaiYing 写给 OpenClaw 的模型能力元数据不可靠。**

OpenClaw gateway 依据 `openclaw.json` 中的 `models.providers[*].models[*].input` 判断模型是否支持图片。如果该字段是 `['text']`，gateway 会在请求进入 LLM 前丢弃图片。

**2026-05-06 补充结论：第一次修复只覆盖了 BaiYing 写配置的链路，还不够。** 最新日志显示，当前 `openclaw.json` 已经包含 `qwen3.6-plus` / `qwen3.6-plus-hzbInner` 的 `input: ['text', 'image']`，但 OpenClaw gateway 仍输出：

```
parseMessageWithAttachments: 1 attachment(s) dropped — model does not support images
provider=baiying-server model=qwen3.6-plus-hzbInner
promptImages=0
```

这说明图片不是在 BaiYing 侧上传或配置写入阶段丢失，而是在 OpenClaw runtime 的 `resolveGatewayModelSupportsImages()` 预检阶段被判定为不支持图片。根因是 OpenClaw 当前版本的 model catalog 可能没有包含显式配置的 provider model，且 provider 比较使用精确字符串；`qwen-portal` 与 OpenClaw 原生 `qwen` 没有归一化到同一 provider，catalog miss 后函数直接返回 `false`。

**2026-05-06 补充结论 B：qwen3.6 Plus 完成后 UI 仍 loading 不是图片传输问题本身。** 现场唯一 `running` 会话 `82d47ef0-dc99-4d04-99aa-967815af76cb` 的 assistant 消息已写入 `metadata: { isStreaming:false, isFinal:true }`，OpenClaw 日志也显示同一 run 已 `run_completed`。随后 gateway 又发出同一 run 的迟到 `stream=lifecycle, phase=fallback` 事件。BaiYing 旧逻辑会在没有 active turn 时把任何非 error agent event 当作 follow-up turn 重新创建，且 `phase=fallback` 没有 handler，最终把本地 session 状态重新置为 `running`，但再也没有 final/end 事件来收尾。

| 场景 | 错误表现 | 根因 |
|---|---|---|
| `baiying-server/qwen3.6-plus-hzbInner` | 模型不在 provider 注册表里，图片被丢弃 | `baiying-server` 只注册默认模型，未合并服务端全量模型 |
...
3. **服务端模型按全量列表合并。** `baiying-server` 不再只注册当前默认模型。
...
7. **迟到 lifecycle 事件不能改变已完成状态。** `phase=fallback` 只作为诊断/兼容事件处理，不能创建或重开 BaiYing turn；刚关闭的 runId 在短时间内作为 tombstone 保留，防止迟到事件绑定到后续 turn。
...
auth:getModels
  ...
  → openclawConfigSync merges all server models into baiying-server provider
  → qwen3.6-plus-hzbInner writes input: ['text', 'image']
...
'baiying-server': {
  models: [
    { id: 'qwen3.5-plus-hzbInner', input: ['text', 'image'] }
  ]
}
```
...
`tryBaiyingServerFallback()` 先构造了只包含默认模型的 provider。`openclawConfigSync` 后续看到 `baiying-server` provider 已存在，就跳过了服务端全量模型合并。

#### 修复

1. `tryBaiyingServerFallback()` 使用缓存的服务端全量模型构造 fallback provider
2. `openclawConfigSync` 对 `baiying-server.models` 做 upsert，而不是 `if exists then skip`
...
```typescript
models.providers['baiying-server'].models[] = {
  id: 'qwen3.6-plus-hzbInner',
  input: ['text', 'image']
}
```
...
| `src/main/libs/openclawConfigSync.runtime.test.ts` | 回归测试 qwen、custom、baiying-server 的视觉能力配置 |
...
| `src/main/libs/agentEngine/openclawRuntimeAdapter.ts` | BaiYing runtime lifecycle 防护：忽略迟到 fallback，tombstone 已关闭 runId |
...
```
provider=baiying-server/qwen3.6-plus-hzbInner promptImages=1
```
...
| `baiying-server` 服务端返回多个模型 | OpenClaw provider 包含全部服务端模型 |
...
| `baiying-server/qwen3.6-plus-hzbInner` 没进入 catalog 但 config 声明 image | `resolveGatewayModelSupportsImages()` 回退 config，保留图片 |
...
4. `auth:getModels` 失败时，`baiying-server` 只能使用已有缓存或当前模型兜底，无法自动发现新服务端模型。
...
3. 给 `ProviderRegistry` 增加更细的 model alias 规则，例如 `qwen3.6-plus-hzbInner` 与 `qwen3.6-plus` 的关系。
