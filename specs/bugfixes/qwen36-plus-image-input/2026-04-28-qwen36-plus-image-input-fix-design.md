...
1. `baiying-server/qwen3.6-plus-YoudaoInner` 支持图片输入时，OpenClaw 配置必须写出 `input: ['text', 'image']`
...
**2026-05-06 补充结论：第一次修复只覆盖了 BaiYing 写配置的链路，还不够。** 最新日志显示，当前 `openclaw.json` 已经包含 `qwen3.6-plus` / `qwen3.6-plus-YoudaoInner` 的 `input: ['text', 'image']`，但 OpenClaw gateway 仍输出：

```
parseMessageWithAttachments: 1 attachment(s) dropped — model does not support images
provider=baiying-server model=qwen3.6-plus-YoudaoInner
promptImages=0
```
...
| 场景 | 错误表现 | 根因 |
|---|---|---|
| `baiying-server/qwen3.6-plus-YoudaoInner` | 模型不在 provider 注册表里，图片被丢弃 | `baiying-server` 只注册默认模型，未合并服务端全量模型 |
...
```
provider=baiying-server/qwen3.6-plus-YoudaoInner promptImages=1
```
...
| `baiying-server/qwen3.6-plus-YoudaoInner` 没进入 catalog 但 config 声明 image | `resolveGatewayModelSupportsImages()` 回退 config，保留图片 |
