...
用户在 Cowork 任务中调用 browser 工具时，BaiYing 已经启动了一个独立的托管 Chrome profile：`openclaw`。这个托管浏览器启动后会先显示一个空白 tab，随后 browser 工具通过 CDP 打开真实目标页面。
...
- `main-2026-05-20.log` from `baiying-logs-20260520-233649`
- `openclaw-2026-05-20.log` from `baiying-logs-20260520-233649`
- `gateway-2026-05-20.log` from `baiying-logs-20260520-233649`

任务标识：

```text
runId=0a55a1b0-b895-4a0f-beb3-c30467a60280
sessionKey=agent:main:baiying:f90336eb-3ebc-4045-9eeb-b823a3e81e48
```
...
**Then** BaiYing 可以启动一个 `openclaw` managed Chrome，并通过 CDP 打开目标页面。
...
- BaiYing 补丁：`scripts/patches/v2026.4.14/openclaw-browser-duplicate-launch.patch`
...
由于 BaiYing 通过 `scripts/apply-openclaw-patches.cjs` 对 pinned OpenClaw 版本应用补丁，修复不应只修改 `vendor/openclaw-runtime/*/dist`。
...
3. 将 OpenClaw browser 侧修复向上游同步，减少 BaiYing 长期维护补丁的成本。
