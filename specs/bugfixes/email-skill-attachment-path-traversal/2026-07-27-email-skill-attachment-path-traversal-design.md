...
NVDB 人工智能产品安全漏洞专业库针对 BaiYing 邮箱 Skill 报告了一个路径遍历漏洞，
...
用户配置邮箱后，如果让 BaiYing 下载该邮件附件，邮箱 Skill 会把邮件提供的附件名直接
拼接到目标目录并写盘。攻击者可以利用 `../` 或 `..\` 逃离预期附件目录，在 BaiYing
...
4. BaiYing 当前系统用户对目标位置有写权限。

以下因素降低了直接利用概率：

- 邮件接收本身不会自动写附件；
- 写入权限不高于 BaiYing 当前系统用户；
