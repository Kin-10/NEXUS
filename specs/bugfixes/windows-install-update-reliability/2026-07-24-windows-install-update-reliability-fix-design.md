...
BaiYing 2026.7.23 的 Windows 安装器在部分用户机器上无法完成安装。
问题同时出现在以下两类场景：
...
```text
The BaiYing update stopped because user skills could not be backed up.
...
2. 已验证的单一 BaiYing 残留可 repair-in-place；合法换目录可
...
| 数据 | 当前权威位置 | 安装器处理原则 |
|---|---|---|
| 用户 Skills | `%APPDATA%\BaiYing\SKILLs` | 安装和更新不得覆盖或删除 |
...
否则“注册异常但可验证的 BaiYing 安装”会被统一归入 orphaned/cross-scope
...
| 无注册，但目标是唯一、可验证的 BaiYing footprint | `repair-in-place` |
...
“注册项不存在但目标目录非空”不得归类为 fresh。可验证且唯一的 BaiYing
...
**Given** 用户没有已安装的 BaiYing，Windows PowerShell 系统文件存在，
...
**And** trusted handoff 已绑定 original SID 时，BaiYing 可同时导出用户
目录和其有权只读的公共日志；无绑定时提示需提权收集
**And** 不把公共 `%APPDATA%` 当作当前用户的正式 Skills 根目录。

### 场景 10：用户卸载但保留数据

**Given** 用户因安装故障卸载 BaiYing
**When** 用户没有明确选择删除数据
**Then** `%APPDATA%\BaiYing` 中的 Skills、SQLite 和 OpenClaw state 保留
**And** 修复版重新安装后可继续使用原数据。

### 场景 11（P0.5）：卸载后目标目录仍有残留

**Given** 注册项已经不存在，但目标目录仍包含 BaiYing 文件或未知内容
**When** 用户重新运行修复版安装包
**Then** 安装器不得把它识别为 `fresh-install`
**And** 唯一、可验证且 content guard 允许的 BaiYing 残留进入
`repair-in-place`
...
完整动作。该判断必须位于当前
`stopBaiYingProcesses` 和 Skills PowerShell 调用之前。

### FR-2：全新安装不得执行旧数据保护流程

`fresh-install` 必须满足：

- 不调用停止旧 BaiYing 的外部命令；
...
- `repair-in-place`：无注册，但 target 是唯一可验证 BaiYing footprint；
...
- 当前用户安装：固定 `%APPDATA%\BaiYing\Installer`；
- 全用户安装：固定 `%ProgramData%\BaiYing\Installer`；
- 不论最终 scope，任何驱动提权 commit/rollback 的 watchdog/control 状态
  都固定写 `%ProgramData%\BaiYing\Installer\control\<attemptId>`，
  绝不写普通用户可修改的 `%APPDATA%`；
...
```text
%ProgramData%\BaiYing\Installer\control\<attemptId>\
%ProgramData%\BaiYing\Installer\logs\<attemptId>\
```
...
3. 旧主程序位于 action plan 的 canonical old source，普通文件、非
   reparse point，且旧 BaiYing footprint 验证通过；
...
- 发布构建必须签名；
- 只接受受支持的子命令；
- 删除/停止操作只允许作用于规范化后的 BaiYing 安装根；
...
```text
%ProgramData%\BaiYing\Installer\legacy-skills\<attemptId>\<rootId>
```
...
| 另一目录有运行中 BaiYing | 正常 | 无 | standalone / interactive | 覆盖目标目录 | 不误停另一目录进程 |
...
4. 临时 PATH 注入不是通用支持方案。普通非提权 CMD 启动
   ...
   where powershell.exe
   "D:\Download\BaiYing Setup 2026.7.23.exe"
   ```
