...
BaiYing 的 DeepSeek V4 Flash / Pro 长会话出现 prompt cache 命中率从 90% 以上下降到约 50% 的情况。用户侧表现为短输入和连续工具调用仍产生较高积分消耗。
...
4. 本分支先前移植的 [openclaw/openclaw#95311](https://github.com/openclaw/openclaw/pull/95311) 尚未证明能解决 BaiYing 的实际坏 case，且会改变 DeepSeek 请求布局，因此本轮已移除。
...
- [netease-youdao/BaiYing#2219](https://github.com/netease-youdao/BaiYing/pull/2219)
...
4. 同时覆盖直接 DeepSeek 和 `baiying-server` 套餐 DeepSeek V4 模型。
...
```powershell
Select-String -Path "$env:APPDATA\BaiYing\openclaw\logs\gateway-*.log" -Pattern '\[DeepSeekCacheProbe\]'
```
...
| `firstDiff=-1` 且 `toolsHash` 不变 | BaiYing payload 前缀稳定，继续排查套餐代理或 DeepSeek cache backend |
