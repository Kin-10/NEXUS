...
关键边界是：Copilot API token 是短期运行时凭据，约 30 分钟刷新一次；它不应作为长期配置写入 `app_config`。OpenClaw Gateway 侧当前通过本地兼容代理访问 Copilot，配置里使用稳定的 `baiying_PROXY_TOKEN`，并不依赖 `app_config.providers.github-copilot.apiKey` 中的短期 token。
...
- `resolveApiKey()` 使用 `${baiying_PROXY_TOKEN}`。
...
5. OpenClaw Gateway 的 GitHub Copilot provider 仍使用本地 proxy 与 `${baiying_PROXY_TOKEN}`。
...
- 覆盖 Copilot provider 的 OpenClaw config 仍输出 proxy base URL 与 `${baiying_PROXY_TOKEN}`。
