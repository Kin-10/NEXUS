...
用户反馈 BaiYing 的 Windows 更新脚本仍在使用 VBScript，而 Windows 11 正在逐步淘汰 VBScript 支持。截图中出现两个关键信息：

1. Windows 兼容性提示检测到 `Wscript.exe` VBScript 使用。
2. Windows Script Host 报错无法找到临时文件中的 `VBScript` 脚本，路径形如 `%TEMP%\baiying-update-<timestamp>.vbs`。

代码现状与反馈一致。Windows 更新安装路径在 `src/main/libs/appUpdateInstaller.ts` 的 `installWindowsNsis()` 中会：

1. 生成 `%TEMP%\baiying-update-<timestamp>.ps1`。
2. 生成 `%TEMP%\baiying-update-<timestamp>.vbs`。
3. 通过 `wscript.exe` 运行 `.vbs`。
4. `.vbs` 再隐藏启动 `powershell.exe`，由 PowerShell 等待 BaiYing 退出后打开 NSIS 安装器。
...
因此用户说的“BaiYing 还在使用 vbs”是准确的。问题不在 NSIS 安装包本身，也不是手动下载链接，而是应用内“下载完成后点击立即更新”的 Windows 安装启动链路。
...
1. NSIS 安装器需要在 BaiYing 当前进程退出后再运行，避免安装时文件仍被占用。
2. 等待和启动安装器的辅助脚本不应显示额外控制台窗口。

为此现有实现使用了“两层脚本”：

```text
BaiYing main process
  -> wscript.exe baiying-update-<ts>.vbs
    -> powershell.exe -WindowStyle Hidden -File baiying-update-<ts>.ps1
...
**Given** 当前 BaiYing 进程退出需要清理 OpenClaw Gateway、IM Gateway 或文件句柄  
**When** launcher 已启动  
...
```text
%TEMP%\baiying-update-<timestamp>.vbs
```
...
- 保留 `detached: true` 和 `unref()`，使等待脚本不依赖 BaiYing 主进程生命周期。
...
1. 写入 `%TEMP%\baiying-update-<timestamp>.log`。
2. 记录当前 app PID。
3. 最多等待当前 PID 退出。
...
| PowerShell 已启动但脚本内部失败 | 写入 `%TEMP%\baiying-update-<ts>.log`，现有 UI 可能已退出；这是当前架构限制，可通过日志诊断 |
...
1. 准备一个可检测到新版本的 BaiYing build。
2. 等待或手动触发更新下载。
3. 点击立即更新。
4. 确认不会出现 Windows 兼容性助手的 `Wscript.exe VBScript` 提示。
5. 确认不会出现 Windows Script Host 的 `.vbs` 错误。
6. 确认 `%TEMP%` 下没有新生成的 `baiying-update-<ts>.vbs`。
7. 确认 `%TEMP%\baiying-update-<ts>.ps1` 和 `.log` 存在。
8. 确认 BaiYing 退出后 NSIS 安装器正常出现。
