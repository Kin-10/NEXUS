...
当 `getOpenClawTokenProxyPort()` 可用时，`baiying-server` provider 的 OpenClaw 配置使用 `${LOBSTER_PROXY_TOKEN}`，不再引用 `${LOBSTER_APIKEY_SERVER}`。
...
6. token proxy 不可用的回退场景下，`baiying-server` 仍能通过 `${LOBSTER_APIKEY_SERVER}` 工作。
7. 单测覆盖：
...
| `src/main/libs/openclawConfigSync.ts` | `baiying-server` provider 已使用 token proxy 与 `${LOBSTER_PROXY_TOKEN}` |
