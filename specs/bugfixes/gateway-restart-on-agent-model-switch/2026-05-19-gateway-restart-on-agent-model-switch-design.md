...
用户只是在 Cowork 输入框里切换了一下模型，OpenClaw gateway 就被 BaiYing 主进程主动重启。
...
2026-05-19T00:27:31.263+08:00 modified: baiying_APIKEY_SERVER prev=... next=...
...
2. **未实际使用的 `baiying_APIKEY_SERVER` 仍参与 secret env diff**

   `baiying-server` 当前已经通过 token proxy 供 OpenClaw 访问。`openclaw.json` 中该 provider 的配置在 token proxy 可用时使用：

   ```text
   baseUrl: http://127.0.0.1:<token-proxy-port>/v1
   apiKey: ${baiying_PROXY_TOKEN}
   ```

   但 `resolveAllProviderApiKeys()` 仍把当前登录态 `accessToken` 收集为 `SERVER`，随后 `collectSecretEnvVars()` 注入 `baiying_APIKEY_SERVER`。当登录 token 因 401 被动刷新或主动刷新发生变化后，下一次任意配置同步都会发现：

   ```text
   baiying_APIKEY_SERVER prev != next
   ```
...
核心问题不是模型切换本身，而是 **动态 token 已迁移到 token proxy 后，旧的 `baiying_APIKEY_SERVER` 仍被当成 gateway 进程启动环境的一部分参与重启判断**。
...
### 场景 C: baiying-server accessToken 刷新

**Given** `baiying-server` 通过 token proxy 访问真实服务端  
**When** 登录 accessToken 因主动刷新或 401 被动刷新发生变化  
**Then** token proxy 使用最新 token 转发请求  
**And** 不因为 `baiying_APIKEY_SERVER` 变化重启 gateway
...
### FR-1: token proxy 可用时不注入动态 `baiying_APIKEY_SERVER`

当 `getOpenClawTokenProxyPort()` 可用时，`baiying-server` provider 的 OpenClaw 配置使用 `${baiying_PROXY_TOKEN}`，不再引用 `${baiying_APIKEY_SERVER}`。

此时 `collectSecretEnvVars()` 不应收集当前登录 accessToken 到 `baiying_APIKEY_SERVER`，避免 rolling token 成为 gateway 重启条件。

### FR-2: 保留 token proxy 不可用时的兼容路径

如果 token proxy 未启动或不可用，`baiying-server` 仍可回退到旧路径：

```text
apiKey: ${baiying_APIKEY_SERVER}
```

只有这种情况下，才需要把 `accessToken` 作为 `baiying_APIKEY_SERVER` 注入 gateway 环境，并允许它参与 hard restart 判断。
...
1. `baiying_APIKEY_SERVER` 这类已迁移但仍残留在收集逻辑里的变量
...
### 4.1 去掉 token proxy 模式下的 `baiying_APIKEY_SERVER`
...
| `src/main/libs/openclawConfigSync.test.ts` | 增加 token proxy 模式下不注入 `baiying_APIKEY_SERVER` 的单测 |
...
注意 `claudeSettings.ts` 当前已经有注释说明 `baiying-server token is now managed by the token proxy`，但实现仍返回 `SERVER`。修复时应让代码与注释一致。
...
| 场景 | 处理方式 |
|---|---|
| token proxy 正常运行 | 不注入 `baiying_APIKEY_SERVER`，不因 accessToken rolling refresh 重启 |
| token proxy 启动失败 | 回退注入 `baiying_APIKEY_SERVER`，保持旧路径可用 |
...
| 历史 `openclaw.json` 仍引用 `${baiying_APIKEY_SERVER}` | 自动把该变量纳入比较和注入，保持兼容 |
| `baiying_PROVIDER_API_KEY` legacy var 变化 | 若配置不再引用它，不应触发 hard restart |
...
4. accessToken 被动刷新或主动刷新后，再切换 Agent 模型，不因 `baiying_APIKEY_SERVER` 变化触发重启。
5. 修改真实自定义 provider API key 时，仍能触发 hard restart，并且重启后请求使用新 key。
6. token proxy 不可用的回退场景下，`baiying-server` 仍能通过 `${baiying_APIKEY_SERVER}` 工作。
7. 单测覆盖：
   - token proxy 模式不收集 `baiying_APIKEY_SERVER`
...
| `src/main/libs/openclawConfigSync.ts` | `baiying-server` provider 已使用 token proxy 与 `${baiying_PROXY_TOKEN}` |
