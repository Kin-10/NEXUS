...
用户在 BaiYing 的 Agent 设置页修改“身份描述”后，当前 Agent workspace 中的
...
| `USER.md` | 关于用户的信息 |
| `AGENTS.md` | workspace 规则、技能策略、运行策略和 BaiYing managed section |
...
```text
<!-- BaiYing managed: do not edit below this line -->
```
...
本修复只处理 BaiYing 历史生成的高置信 legacy identity block：

- 位于 `AGENTS.md` 的 BaiYing managed marker 之前；
- 位于文件顶部默认模板区域；
...
**And** `AGENTS.md` 其它用户内容和 BaiYing managed section 保持不变
...
```text
<agent-workspace>/.baiying/migrations/
```
...
- `.baiying/migrations/` 不属于 OpenClaw bootstrap 文件名，避免再次注入 prompt；
...
- 保留 marker 前用户内容；
- 写入 marker 后 BaiYing managed section；
...
2. 同一次保存后，如果 `AGENTS.md` 顶部存在高置信 `## Identity（必须遵守）` 历史块，该块
   从 prompt 表面移除。
3. 清理后的 `AGENTS.md` 仍保留原来的默认模板内容、用户其它规则和 BaiYing managed
   section。
4. 清理前完整备份原始 `AGENTS.md` 到 `<workspace>/.baiying/migrations/`。
...
5. 确认 `.baiying/migrations/` 下存在原始 `AGENTS.md` 备份。
