# BaiYing 数据备份与跨机器还原设计文档

## 1. 概述

### 1.1 问题/背景

BaiYing 需要在设置页提供“数据备份”和“数据迁移”能力，用于用户把一台机器上的 BaiYing 迁移到另一台机器。应用内迁移采用新的归档格式，只支持由 BaiYing 设置页导出的备份包。

应用内迁移不能简单采用“强制退出后打包/删除/解压”模型，因为主进程、SQLite、OpenClaw gateway、定时任务和 IM 网关都可能在运行中写入数据。已经出现过以下失败现象：

- 备份包里缺少或包含错误的 `baiying.sqlite`，导致多 Agent、历史记录、定时任务、自定义模型 API key、IM 配置没有迁移成功。
...
3. 备份必须包含 BaiYing 的核心状态：登录态、Agent、会话历史、定时任务、自定义模型、IM 配置、技能、插件/MCP 状态和 OpenClaw state。
...
**When** 用户在旧电脑设置页点击“备份数据”，把生成的 `baiying-backup-yyyyMMdd-HHmmss.tar.gz` 拷贝到新电脑，并在新电脑设置页点击“导入备份”。  
**Then** 新电脑重启后能看到同样的 Agent、历史记录、自定义模型、IM 配置和定时任务。

### 场景 2: 从 macOS 迁移到 Windows

**Given** macOS 上的 BaiYing 已使用一段时间，包含 SQLite 数据、OpenClaw state、技能和插件状态。  
...
```text
baiying-backup-yyyyMMdd-HHmmss.tar.gz
```

备份期间显示全局 loading，文案说明正在备份 BaiYing 数据，关闭应用会中断备份。备份成功后展示备份路径、文件大小，并提供“在文件夹中显示”。

### FR-2: 导入入口

同一位置提供“导入备份”按钮。点击后弹出选择归档对话框，支持 `.tar.gz`、`.tgz`。导入前必须弹出确认弹窗，说明：

- 当前 BaiYing 数据会被备份包整体替换。
- 应用会先自动生成回滚备份。
- 导入期间不要关闭应用。
- 成功后应用会自动重启。
...
```text
BaiYing/.baiying-migration.json
```
...
- staged `baiying.sqlite` 来自 SQLite backup API 快照，而不是 live 文件直接复制。
...
- 恢复后的目标 `baiying.sqlite` 与归档源 SQLite 的内容摘要一致。
...
#### SQLite: `baiying.sqlite`

SQLite 是 BaiYing 的主要权威数据源，必须通过 SQLite backup API 生成一致快照后迁移。至少包括：
...
| `Cookies*`、`DIPS*`、`Network/Cookies*` | Windows 上经常被 Chromium 锁定，且 BaiYing 登录态应以 SQLite token 为准 |
...
```text
baiying-backup-20260609-103000.tar.gz
└── BaiYing/
    ├── .baiying-migration.json
    ├── baiying.sqlite
    ├── SKILLs/
...
归档中不应包含 live `baiying.sqlite-wal` 和 `baiying.sqlite-shm`。如果导入预检发现归档中包含 SQLite sidecar 文件，必须先在 staging 中 checkpoint/合并后再恢复主库。
...
staging 目录中的 `baiying.sqlite` 必须来自该快照，不能来自 live userData 的直接复制。

### 6.4 文件 staging

1. 创建临时 staging 目录。
2. 按“必须迁移的数据”和“默认排除的数据”递归复制 `userData`。
3. 不复制 live `baiying.sqlite`、`baiying.sqlite-wal`、`baiying.sqlite-shm`。
4. 将 SQLite 快照复制为 `stage/BaiYing/baiying.sqlite`。
...
1. 基于 staged 数据生成 `.baiying-migration.json`。
...
6. staging 提取后必须找到 `baiying.sqlite`。
7. 对 staging SQLite 执行 `PRAGMA quick_check`。
8. 如果 staging 中存在 `baiying.sqlite-wal`，先打开 staging 数据库执行 checkpoint，合并 WAL，再删除 sidecar。
...
4. 立即打开一个不使用 BaiYing `userData` 的专用恢复进度窗口，使用非持久化 session/partition，只展示 loading 和安全提示。
...
```text
baiying-rollback-yyyyMMdd-HHmmss.tar.gz
```
...
4. SQLite 使用专门流程：
   - 确认 SQLite store 已关闭。
   - 删除目标 `baiying.sqlite-wal` 和 `baiying.sqlite-shm`。
   - 复制 staging `baiying.sqlite` 到目标。
   - 不复制 staging sidecar。
...
```text
userData/.baiying-restore-result.json
```
...
BaiYing 服务登录态应以 SQLite 中的 token 为准，可以跨机器恢复，但 refreshToken 过期时仍需要重新登录。
...
macOS 同机重装通常不会删除 `~/Library/Application Support/BaiYing`，但跨机器迁移仍需本功能。macOS 也存在 WAL-only 数据丢失风险，所以必须使用同样的 SQLite 快照和 checkpoint 验证流程，不能因为 macOS 文件锁较少就直接复制 live 数据库。
...
1. 备份包中必须存在 `BaiYing/baiying.sqlite` 和 `BaiYing/.baiying-migration.json`。
2. 新格式备份包不得包含 `baiying.sqlite-wal`、`baiying.sqlite-shm`、`backups/`、`sqlite-backups/`、`runtimes/`、Chromium cache、日志和 lock 文件。
