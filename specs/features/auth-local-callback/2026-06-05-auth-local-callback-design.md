...
客户端点击登录
  ...
  -> 用户在网页端完成登录
  -> 网页触发 baiying://auth/callback?code=<authCode>
  -> 浏览器弹出系统确认框，用户点击"打开 BaiYing"
  -> Electron 通过 deep link 收到 code
  -> 主进程调用 /api/auth/exchange 换取 accessToken 与 refreshToken
```

该流程可用，但浏览器在打开 `baiying://` 自定义协议时会显示系统级确认弹窗。弹窗由浏览器控制，样式不可定制；首次登录时用户会看到一层额外确认，体验不够顺滑。

本需求希望改为 **localhost / 127.0.0.1 本地回调模式**：网页登录成功后直接重定向到客户端临时启动的本地 HTTP callback server，由客户端接收一次性 auth code 并完成登录。因为浏览器访问的是普通 HTTP 地址，而不是外部应用协议，所以不会触发"是否打开 BaiYing"的系统确认框。
...
- 保留现有 `baiying://auth/callback` deep link 逻辑作为兼容或 fallback。
- 复用现有 `/api/auth/exchange`、token 持久化、刷新、用户信息加载逻辑。
- 网页端登录成功后展示用户友好的成功页，提示可以关闭浏览器页面或返回 BaiYing。
- 客户端收到本地 callback 后主动恢复并聚焦 BaiYing 主窗口，尽量让用户回到应用。
...
**When** 用户点击登录
**Then** 客户端可继续使用现有 `baiying://auth/callback` deep link 流程作为 fallback，确保登录功能不回退
...
- 保留现有 `baiying://auth/callback?code=...` 解析。
...
参考实现可借鉴 `src/main/libs/openaiCodexAuth.ts` 中本地 callback server 的 HTML 响应、超时和 server close 模式，但端口策略不同：BaiYing 登录使用动态端口；OpenAI Codex OAuth 因第三方注册限制必须使用固定 `1455`。
...
实际实现时需要给 `AuthCallbackRouter` 增加一个直接投递 code 的方法，避免为了本地 callback 再伪造 `baiying://` URL。例如：
...
- 同一时间只允许一个 BaiYing 登录 callback server。
- 如果用户重复点击登录：
  - 关闭旧 server；
  - 创建新 state 和新 server；
  - 打开新的网页登录 URL。
...
- 成功：`登录已完成，正在返回 BaiYing 登录页。`
- 失败：`登录失败，请返回 BaiYing 后重试。`
...
```text
baiying://auth/callback?code=<authCode>
```
...
`/Users/admin/Desktop/disk/work/baiying-portal`
...
function resolveElectronCallbackUrl(authCode: string): string {
  ...
  return `baiying://auth/callback?code=${encodeURIComponent(authCode)}`;
}
```
...
|------------|----------------|------------|
| 旧版本 | `#/login?source=electron` | 没有 `redirect_uri`，继续使用 `baiying://auth/callback?code=...` |
...
| `/Users/admin/Desktop/disk/work/baiying-portal/src/views/LoginView.vue` | 修改，优先使用 `redirect_uri` 通知新客户端，并保留 deep link 兼容旧客户端 |
| `/Users/admin/Desktop/disk/work/baiying-portal/docs/server-integration/*` | 可选修改，补充 portal 与后端对 `redirect_uri` / `state` 的协作说明 |
...
8. 在 portal 登录页使用 `#/login?source=electron`，普通 URS 登录成功后仍触发 `baiying://auth/callback?code=...`。
9. 员工 OpenID 登录成功后同样遵循 `redirect_uri` 优先、deep link fallback 的规则。
10. 新本地 callback 收到合法 code 后，BaiYing 主窗口会从最小化/后台状态恢复到前台。

### 8.3 回归测试

1. 现有 `baiying://auth/callback?code=...` deep link 仍可触发登录。
...
1. 用户点击客户端登录后，浏览器打开网页登录页，不直接触发浏览器"打开 BaiYing"系统确认框。
...
6. 验收通过后，将网页登录完成页中的"打开 BaiYing"引导降级为 fallback 入口。
