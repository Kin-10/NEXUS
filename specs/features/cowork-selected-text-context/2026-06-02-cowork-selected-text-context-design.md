# BaiYing Cowork 选中文本添加到对话设计文档

## 1. 概述

### 1.1 问题/背景

BaiYing Cowork 当前支持用户直接输入问题、添加文件和图片附件、选择 Skill / Kit，并通过 OpenClaw Gateway 发起对话。在阅读 assistant 回复时，用户经常需要针对其中一小段内容继续追问，例如：
...
BaiYing 可以参考这一交互，但必须适配现有 OpenClaw 接入方式。

### 1.2 OpenClaw 能力确认

BaiYing 当前固定使用 OpenClaw `v2026.4.14`，并通过 Gateway RPC `chat.send` 发起 Cowork turn。
...
因此，BaiYing 不能直接向 `chat.send` 增加以下自定义字段：
...
- `chat.history` 用于读取展示归一化后的历史，不提供 BaiYing 自定义 UI 引用结构。
...
1. BaiYing SQLite 保存结构化文本片段，服务于 UI 恢复、展开和来源定位。
...
5. 支持在同一次追问中添加多个文本片段。
6. 支持查看和移除已选片段。
7. 发送消息时，把结构化片段保存到 BaiYing SQLite 的 user message metadata。
8. 发送给 OpenClaw 时，把片段序列化为 `chat.send.message` 中的上下文区块。
9. 历史 user message 中展示“已选文本片段”标记，并支持展开查看快照。
10. 保持 user message 主体简洁，不在 BaiYing 对话气泡中重复渲染发送给 OpenClaw 的包装文本。
...
- 不在 OpenClaw transcript 中保存 BaiYing 的 `sourceMessageId` 或 DOM offset。
...
- BaiYing 重新打开历史会话后能恢复片段 chip 和来源关系。

### 2.2 BaiYing 与 OpenClaw 的历史差异

同一个用户 turn 在两侧的持久化形态不同：

| 存储位置 | `content` | metadata | 用途 |
|---------|-----------|----------|------|
| BaiYing SQLite | 用户输入的原始请求，例如 `这个是什么意思？` | 结构化 `selectedTextSnippets` | 本地 UI、历史恢复、来源定位 |
| OpenClaw transcript | 片段区块 + 当前请求 | OpenClaw 自身字段 | 模型上下文、OpenClaw 后续历史 |

这是有意设计，不要求两侧 `content` 字符串完全一致。

BaiYing 当前 managed Cowork session 已经采用类似边界：
...
**When** 用户点击 `添加到对话`
**Then** BaiYing 拒绝添加并展示 i18n toast
...
| 字段 | 说明 |
|------|------|
| `id` | BaiYing 生成的片段 id，用于草稿列表 key 和删除 |
...
3. 把片段保存到 BaiYing user message metadata。
...
- 不把 `sourceMessageId` 发给模型，避免注入无意义的 BaiYing 内部 id。
- 片段区块只在本次 user turn 中追加一次。
- 不把序列化后的包装文本回写到 BaiYing user message `content`。
...
第一版只对 BaiYing managed Cowork session 开放文本选择入口。

原因：

- managed Cowork session 中，BaiYing SQLite 是 UI 展示的主要来源。
- OpenClaw transcript 保存序列化后的包装文本，用于模型上下文。
- IM / channel session 会通过 `chat.history` reconcile 覆盖本地 user / assistant 消息。
- `chat.history` 只能恢复包装后的 user message 字符串，不能恢复 BaiYing 专用 `sourceMessageId` 与片段列表。
...
- BaiYing 的版本 patch 只增加 `cwd?: string`。
...
5. BaiYing 对应版本 patch 中的 `chat.send` 改动
...
```text
1. BaiYing system instructions（如有变化）
...
2. BaiYing patch 只新增 `cwd?: string`，没有文本引用协议。
...
| OpenClaw `chat.history` 截断长消息 | BaiYing managed session 仍从本地 SQLite 展示干净 user message |
...
- BaiYing SQLite user message `content` 仍为原始用户请求。
...
5. 查看 BaiYing SQLite / UI，确认保存和展示的是原始问题 + snippets metadata。
...
- [ ] BaiYing SQLite user message `content` 只保存用户原始请求。
- [ ] BaiYing SQLite user message metadata 保存结构化 `selectedTextSnippets`。
...
5. OpenClaw runtime 仍使用 `v2026.4.14` 与现有 BaiYing patch，不需要额外 rebuild 语义变化。
