...
Add a "prevent sleep" toggle to Settings > General tab, preventing the system from sleeping while BaiYing is running. Follows the existing autoLaunch pattern.

## User Story

As a user running long AI tasks or IM bots, I want BaiYing to prevent my computer from sleeping, so tasks aren't interrupted by the system going to sleep.
...
**Goal:** Add a "prevent sleep" toggle in Settings > General that keeps the system awake while BaiYing is running.
