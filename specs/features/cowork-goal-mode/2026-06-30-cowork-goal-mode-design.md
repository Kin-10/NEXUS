...
BaiYing currently treats Cowork sessions as ordinary chat sessions and does
...
Add a BaiYing client goal mode that mirrors OpenClaw's session-goal model:
...
6. Keep the model-side goal tools available through OpenClaw; BaiYing should
...
- BaiYing will not create an independent goal persistence layer while
  OpenClaw owns goal state.
- BaiYing does not expose `/goal` command text as normal user-visible chat
...
OpenClaw remains authoritative. BaiYing normalizes the `goal` field from
...
For existing sessions, BaiYing sends goal mutations through the OpenClaw
