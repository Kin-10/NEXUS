# BaiYing ToB Enterprise Config Support

## Overview

Enable BaiYing to support enterprise (ToB) deployment by reading a pre-configured package on startup, syncing model/IM/skill/agent/MCP configurations into SQLite, and locking down the UI so users can work immediately without manual setup.
...
**Platform paths**:
- macOS: `~/Library/Application Support/BaiYing/enterprise-config/`
- Windows: `%APPDATA%/BaiYing/enterprise-config/`
...
**Goal:** Enable BaiYing to read a pre-configured enterprise package on startup, sync model/IM/skill/agent/MCP configs into SQLite, and lock down the UI — so enterprise users can run the app with zero manual setup.
...
```bash
mkdir -p ~/Library/Application\ Support/BaiYing/enterprise-config/skills/test-skill
mkdir -p ~/Library/Application\ Support/BaiYing/enterprise-config/agents
mkdir -p ~/Library/Application\ Support/BaiYing/enterprise-config/mcp
```
...
```bash
rm -rf ~/Library/Application\ Support/BaiYing/enterprise-config
```
