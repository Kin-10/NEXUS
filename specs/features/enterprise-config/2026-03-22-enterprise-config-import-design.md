...
BaiYing needs enterprise batch deployment support: IT/ops teams push a unified config file to multiple machines, and BaiYing loads it on startup. The config file reuses OpenClaw's `openclaw.json` format so that ops teams maintain a single config for both OpenClaw and BaiYing deployments.

## Usage

```bash
baiying --config /opt/baiying/enterprise.json
```
...
```
Startup: baiying --config /path/to/config.json
  │
...
OpenClaw uses plugin-style channel keys that differ from BaiYing's internal platform names. The importer accepts **both** forms for flexibility:

| openclaw.json channel key | BaiYing im_config key |
|---------------------------|------------------------|
...
```
[ConfigImporter] loading enterprise config from /opt/baiying/enterprise.json
