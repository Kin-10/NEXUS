...
> 状态：待评审  
> 涉及仓库：`BaiYing`  
> 产品入口：BaiYing 首页左侧「Agent 任务列表」与旧版 Cowork 会话列表  
...
当前 IM 渠道创建或同步到 BaiYing 后，会话标题通常通过文字前缀表达来源，例如 `[微信] group:o9cq...`。这能帮助用户识别渠道，但在左侧会话列表中占用有限标题空间，也与 IM 机器人设置页已经使用渠道 logo 的视觉语言不一致。
