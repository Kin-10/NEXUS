...
Codex 的插件引用方式提供了一个更合适的用户体验：用户可见层只显示一个插件引用，例如 `[@github](plugin://github@openai-curated)`，底层包含哪些 skills、MCP tools、apps/connectors 不直接暴露。BaiYing 的 Kit 不应直接等同于 Codex plugin 或 OpenClaw plugin，但可以参考这种"用户可见引用 + 内部能力解析"的分层设计。
...
- 引用至少包含：
  ...
  - `id`: Kit ID
  - `name`: 本地化后的完整名称
  - `uri`: 稳定引用 URI，例如 `kit://github@baiying-kits`
...
- Kit 引用可以序列化为 Markdown 链接，例如 `[@github](kit://github@baiying-kits)`。
...
Codex plugin、OpenClaw plugin、BaiYing Kit 是三个不同概念：
...
| BaiYing Kit | BaiYing | `kit://<id>@baiying-kits` | BaiYing 专家套件，可聚合多类能力 |
...
```text
kit://<kit-id>@baiying-kits
```

示例：

```markdown
[@github](kit://github@baiying-kits)
```
...
| `kit://` 链接被复制到普通 Markdown 环境 | 作为普通链接文本保留，不影响 BaiYing 内部渲染 |
...
9. `kit://` Markdown 链接在 BaiYing 内部渲染为安全的 Kit 引用，不打开外部浏览器。
