...
OpenClaw `v2026.6.1`, the version currently pinned by BaiYing, supports
...
BaiYing does not currently expose that behavior correctly:
...
- BaiYing does not reimplement OpenClaw's context snapshot, model selection,
...
For a valid existing session, BaiYing sends:
...
OpenClaw does not retain the answer as a native side thread. BaiYing keeps
...
BaiYing renders only the final side-result text and existing approval flows
