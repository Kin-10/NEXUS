...
BaiYing 已经在对话中的 `ArtifactPreviewCard` 提供两类发布入口：
...
如果状态数据短暂冲突，以受管 HTML Artifact 身份为优先，因为 HTML preview session 本身使用内部 loopback 服务；只有 `browserHtmlArtifactId` 已清除后，loopback URL 才能作为用户本地服务参与部署识别。这样可以避免把 BaiYing 的 HTML 预览服务误当作用户项目部署。
