...
当前 BaiYing 已支持 HTML Artifact 分享，并在近期扩展了图片和 SVG 分享。分享链路由 `baiying-server` 提供：
...
```text
baiying-server/src/main/resources/static/html-share/office-preview/
```
...
`baiying-server` 的 `HtmlShareService` 新增服务端 sourceType 常量或枚举，并加入 `SOURCE_TYPES`。不要在 controller、mapper 或后台接口中散落 `"document_file"` 裸字符串。
...
```text
baiying-server/src/main/share-preview/
```
...
`baiying-admin/src/views/HtmlShareListView.vue`：
...
`baiying-admin/src/api/htmlShares.ts`：
