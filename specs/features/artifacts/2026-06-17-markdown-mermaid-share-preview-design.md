...
BaiYing 已经基于 `baiying-server` 的 HTML 分享系统支持 HTML、图片、SVG、Office/PDF 文件分享。现有链路为：
...
构建方式参考 `src/main/share-preview/README.md` 中 Office bundle 的方式，使用 BaiYing 客户端已有依赖打包：

```bash
./node_modules/.bin/esbuild ../baiying-server/src/main/share-preview/text-preview.tsx \
  ...
  --outfile=../baiying-server/src/main/resources/static/html-share/text-preview/text-preview.js
```
