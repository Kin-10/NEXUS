...
BaiYing 需要支持渠道包投放统计。不同投放渠道在打包或开发启动时传入不同 `keyfrom`，应用安装并启动后需要把渠道来源稳定记录到本地，并在必要的服务端接口中附带该来源，为后续登录、模型、用户信息、支付、埋点和报表统计提供基础数据。
...
- `/openapi/get/luna/hardware/baiying/{env}/update`
- `/openapi/get/luna/hardware/baiying/{env}/update-manual`
...
```text
Mac Intel:
BaiYing-darwin-x64-<version>-<keyfrom>.dmg

Mac Apple Silicon:
BaiYing-darwin-arm64-<version>-<keyfrom>.dmg

Windows x64:
BaiYing-Setup-x64-<version>-<keyfrom>.exe
```

示例：

```text
BaiYing-darwin-x64-2026.5.14-bilibili.dmg
BaiYing-darwin-arm64-2026.5.14-bilibili.dmg
BaiYing-Setup-x64-2026.5.14-bilibili.exe
```
...
| `/openapi/get/luna/hardware/baiying/{env}/update`        | `GET`        | URL query 追加 `firstKeyfrom`、`latestKeyfrom`、`uuid`、`version`、`userId` |
| `/openapi/get/luna/hardware/baiying/{env}/update-manual` | `GET`        | URL query 追加 `firstKeyfrom`、`latestKeyfrom`、`uuid`、`version`、`userId` |
...
/openapi/get/luna/hardware/baiying/prod/update?uuid=...&userId=...&version=...&firstKeyfrom=bilibili&latestKeyfrom=baidu
/openapi/get/luna/hardware/baiying/prod/update-manual?uuid=...&userId=...&version=...&firstKeyfrom=bilibili&latestKeyfrom=baidu
```
...
4. `registerProxyTokenRefresher('baiying-server')` 代理刷新。
...
- 自动更新检查：`getUpdateCheckUrl()`，正式服为 `/baiying/prod/update`，测试服为 `/baiying/test/update`。
- 手动更新检查：`getManualUpdateCheckUrl()`，正式服为 `/baiying/prod/update-manual`，测试服为 `/baiying/test/update-manual`。
...
16. `/baiying/{env}/update` 和 `/baiying/{env}/update-manual` 保持 GET，只在 query 追加可用的归因参数。
...
1. 双击 `.dmg` 安装包。
2. 在弹出的挂载窗口中找到 `BaiYing.app`。
3. 右键 `BaiYing.app`，选择“显示包内容”。
...
1. 双击 `.exe` 安装包完成安装。
2. 打开资源管理器。
3. 在地址栏输入 `%LOCALAPPDATA%\Programs\BaiYing\resources\keyfrom`。
