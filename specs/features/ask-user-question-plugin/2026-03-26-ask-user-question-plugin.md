...
→ 插件 execute() 通过 HTTP POST /askuser 发送到 BaiYing
...
- `agent:main:baiying:*` → 桌面端 → 注册 AskUserQuestion 工具
...
5. **钉钉/企微**：通过 HTTP API 接入，`messageChannel=webchat`，通过 `sessionKey` 前缀区分（非 `baiying:` 前缀）。
