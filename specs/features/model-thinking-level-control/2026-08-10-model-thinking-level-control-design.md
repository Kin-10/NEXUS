...
>
> 适用范围：BaiYing Electron、`baiying-server` 与 OpenClaw runtime
>
> 目标版本：`2026.8.10`
...
本功能因此采用 server 下发模型级配置、BaiYing 持久化产品等级、OpenClaw 使用运行时别名、server 在最终代理边界消费版本化选项的方案。
...
| `requestCapabilities` | 当前模型接受的 BaiYing 内部请求协议 |
...
2. 当前模型的 `requestCapabilities` 包含 `baiying-options-v1`。
...
`baiying_options` 是 BaiYing 与 server 之间的内部协议，不属于上游 OpenAI 请求。server 的处理约定为：
...
```text
baiying_options.thinking.level
...
SQLite: max
  ...
  → local extension: max
  → baiying_options.thinking.level: max
```
...
`baiying-model-compat` 使用完整 `provider/modelId` 精确匹配 `thinkingProfiles`，只负责等级反向映射和 v1 参数注入。它不负责鉴权、额度、Provider 路由、模型 ID 归一化或任意请求字段透传；未匹配模型保持 passthrough。
...
1. server 必须先支持并消费 v1 参数，再发布 BaiYing。
...
1. 优先由 server 停止下发 `thinkingConfig` 或 `baiying-options-v1`，关闭入口和新协议。
...
| 模块 | 关键入口 |
|---|---|
| 共享协议 | `src/shared/providers/modelThinking.ts`、`baiYingRequestOptions.ts`、`modelRuntimeProfiles.ts` |
...
| 本地扩展 | `openclaw-extensions/baiying-model-compat/` |
...
11. `baiying_options` 永远不转发给上游 Provider。
