...
This root `AGENTS.md` is repository-level guidance for BaiYing. Codex may also
...
BaiYing is an Electron + React desktop application. Its core user-facing
...
`Cowork` is BaiYing's product/session layer. The name is historical: it
started as a Claude Code-like in-house coding assistant, but in the current
codebase it means the BaiYing layer that owns sessions, messages,
...
When changing OpenClaw-related behavior, first look for a BaiYing-side
...
BaiYing when the behavior is product-specific or can be expressed cleanly at
the integration boundary.

Use version-scoped OpenClaw patches only when the required behavior is inside
OpenClaw and there is no clean BaiYing-side hook. Do not avoid a patch by
adding brittle or contorted BaiYing workarounds.
...
- `src/main/libs/openclawConfigSync.ts`: renders BaiYing state into
...
SQLite lives in Electron `app.getPath('userData')` as `baiying.sqlite`.
...
Important paths:
- `%APPDATA%/BaiYing/openclaw/state/openclaw.json` on Windows: generated
  OpenClaw config.
- `%APPDATA%/BaiYing/openclaw/state/workspace-main`: main agent workspace.
- `%APPDATA%/BaiYing/openclaw/state/workspace-{agentId}`: non-main agent
  workspaces.
...
- `AGENTS.md`: OpenClaw workspace instructions with a BaiYing-managed section.
...
Main logs:
- Windows: `%APPDATA%/BaiYing/logs/main-YYYY-MM-DD.log`
...
- Windows: `%APPDATA%/BaiYing/openclaw/logs/gateway-YYYY-MM-DD.log`
- Retention is 3 days.
