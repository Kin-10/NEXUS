...
- `/api/proxy/chat/completions` does not inject cache markers; BaiYing/OpenClaw must send `cache_control` in the upstream payload when explicit cache is enabled.
...
- If server model metadata is not loaded yet, still infer these defaults for `qwen3.5*`, `qwen3.6*`, and `claude-*` BaiYing server model IDs.
