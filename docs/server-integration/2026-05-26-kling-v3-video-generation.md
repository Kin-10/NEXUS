...
baiying-server adds Aliyun Bailian `kling/kling-v3-video-generation` through the existing video media endpoints. The server submits DashScope async tasks, polls `/api/v1/tasks/{task_id}`, and reconciles final credits from upstream `usage.duration`, `usage.SR`, and `usage.audio`.
