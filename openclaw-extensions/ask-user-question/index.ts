 ...
 * and waits for the user's response via an HTTP callback to BaiYing.
 *
 * This enables delete-confirmation modals on the BaiYing desktop app
...
const plugin = {
  ...
  description: 'Structured user confirmation tool for BaiYing desktop.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Use a factory so the tool is only available for BaiYing local-session candidates.
    // IM channel sessions (qqbot, dingtalk, weixin, feishu, etc.) get null → tool hidden.
    api.registerTool((ctx) => {
      // Enable for BaiYing desktop sessions across agents and delegated child sessions.
