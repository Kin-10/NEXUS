export const BaiYingModelRuntimeProfile = {
  MoonshotKimiK3: 'moonshot-kimi-k3',
} as const;

export type BaiYingModelRuntimeProfile =
  typeof BaiYingModelRuntimeProfile[keyof typeof BaiYingModelRuntimeProfile];

export type BaiYingModelProfileMap = Record<string, BaiYingModelRuntimeProfile>;
...
export const parseModelProfileMap = (value: unknown): BaiYingModelProfileMap => {
  if (!isRecord(value)) return {};
  const result: BaiYingModelProfileMap = {};
  for (const [modelRef, profile] of Object.entries(value).sort(([a], [b]) =>
    a.localeCompare(b))) {
    if (
      isModelRef(modelRef)
      && profile === BaiYingModelRuntimeProfile.MoonshotKimiK3
    ) {
...
export const hasModelRuntimeProfile = (
  modelProfiles: BaiYingModelProfileMap,
  provider: string,
  modelId: string,
  profile: BaiYingModelRuntimeProfile,
...
export const resolveModelProfileTransportDecision = (params: {
  ...
  if (!hasModelRuntimeProfile(
    ...
    params.provider,
    params.modelId,
    BaiYingModelRuntimeProfile.MoonshotKimiK3,
  )) {
