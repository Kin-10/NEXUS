...
const PLUGIN_ID = 'baiying-model-compat';
...
const register = (api: OpenClawPluginApi): void => {
  ...
  api.registerProvider({
    id: PLUGIN_ID,
    label: 'BaiYing Model Compatibility',
    hookAliases: ['baiying-server'],
    auth: [],
    ...
    wrapStreamFn: (ctx) => {
      ...
      if (thinkingProfile?.requestOptionsVersion === BAIYING_REQUEST_OPTIONS_VERSION) {
        return createBaiYingRequestOptionsWrapper(
          baseStreamFn,
          resolveBaiYingRequestThinkingLevel(thinkingProfile, ctx.thinkingLevel),
        );
      }
      return baseStreamFn;
    },
...
export default {
  id: PLUGIN_ID,
  name: 'BaiYing Model Compatibility',
  description: 'Applies explicit BaiYing-managed model runtime profiles.',
  register,
};
