...
const models: Model[] = [
  ...
  { id: 'kimi-k2.6', name: 'Kimi K2.6 Server', providerKey: 'baiying-server', isServerModel: true },
];
...
describe('resolveAgentModelSelection', () => {
  ...
  test('resolves same-id server session model to the server model', () => {
    const result = resolveAgentModelSelection({
      sessionModel: 'baiying-server/kimi-k2.6',
    ...
    expect(result.selectedModel?.providerKey).toBe('baiying-server');
