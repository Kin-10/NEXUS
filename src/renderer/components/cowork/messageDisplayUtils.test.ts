...
test('activity indicator status defaults to thinking and escalates for long waits', () => {
  ...
  expect(getActivityIndicatorStatusText(true)).toBe('正在整理桌上的上下文…');
  expect(getActivityIndicatorStatusText(false, true)).toBe('还在跟进，马上就有进展…');
...
test('media polling groups count their polls as steps', () => {
  const polls = [
    activityToolItem('poll-1', 'baiying_video_generate'),
    activityToolItem('poll-2', 'baiying_video_generate'),
    activityToolItem('poll-3', 'baiying_video_generate'),
  ...
  const mediaItem = {
    ...
    group: {
      type: 'media_polling_group',
      toolName: 'baiying_video_generate',
      taskId: 'task-1',
