...
const getModelAnalyticsSource = (model: Model | null): 'package' | 'custom' | undefined => {
  ...
  if (model.isServerModel || model.providerKey === ProviderName.BaiyingServer) {
    return 'package';
  }
  return 'custom';
};
...
const getModelSelectorGroup = (model: Model | null): 'server' | 'user' | undefined => {
  if (!model) return undefined;
  return model.isServerModel || model.providerKey === ProviderName.BaiyingServer ? 'server' : 'user';
};
