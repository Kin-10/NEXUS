...
test('uses the logged-in user and omits empty optional parameters', () => {
  const result = new URL(buildLogUrl(
    {
      action: `${LogReporterActionPrefix.BaiYing}app_started`,
...
test('reports an event through the Electron API bridge', async () => {
  ...
  expect(requestUrl.searchParams.get('action')).toBe('baiying_plan_mode_enabled');
...
test('rejects an event without the BaiYing action prefix before sending', async () => {
  const fetchMock = vi.fn();
  vi.stubGlobal('window', {
    electron: {
      api: {
