...
import { classifyErrorKey, CoworkErrorI18nKey, isBaiYingQuotaExhaustedError } from './coworkErrorClassify';
...
test('billing: BaiYing upstream balance errors do not tell users to recharge', () => {
  expect(classifyErrorKey('insufficient balance', ProviderName.BaiyingServer))
...
test.each([40200, 40201, 40202])('billing: BaiYing user quota code %s still requires quota recovery', (code) => {
  expect(classifyErrorKey(JSON.stringify({ error: { code, message: 'billing failure' } }), ProviderName.BaiyingServer))
...
test('billing: BaiYing free quota exhausted', () => {
  expect(classifyError('免费额度已用完，请升级套餐')).toBe('coworkErrorQuotaExhausted');
});

test('billing: BaiYing daily free quota code exhausted', () => {
  expect(classifyError('{"error":{"message":"今日免费额度已用完","code":40200}}')).toBe('coworkErrorQuotaExhausted');
});

test('billing: BaiYing free quota code exhausted', () => {
  expect(classifyError('{"error":{"message":"免费额度已用完，请升级套餐","code":40201}}')).toBe('coworkErrorQuotaExhausted');
});

test('billing: BaiYing monthly credits exhausted', () => {
  expect(classifyError('本月积分已用完')).toBe('coworkErrorQuotaExhausted');
});

test('billing: BaiYing monthly quota JSON payload', () => {
  expect(classifyError('{"type":"error","error":{"type":"proxy_error","message":"本月积分已用完","code":40202}}')).toBe('coworkErrorQuotaExhausted');
});

test('billing: detects BaiYing quota exhausted for proxy helpers', () => {
  expect(isBaiYingQuotaExhaustedError('monthly credits exhausted')).toBe(true);
  expect(isBaiYingQuotaExhaustedError('Request failed with status 402')).toBe(false);
});
