/**
 * Where the failing model comes from in BaiYing terms, so users can tell at
 * a glance whether an error concerns the BaiYing plan, a vendor coding plan
 * they configured, or their own custom provider.
 */
export const CoworkErrorModelSource = {
  BaiYingPlan: 'baiying-plan',
...
export interface CoworkErrorDetail {
  /** Runtime error message before BaiYing i18n normalization. */
  rawErrorMessage?: string;
  provider?: string;
  model?: string;
  /** BaiYing-side classification of where the failing model comes from. */
