...
describe('AI Skin Designer built-in kit', () => {
  test('publishes only the bundled skin creator skill', () => {
    ...
    expect(kit.tryAsking.map(prompt => prompt.zh)).toEqual([
      '请为 BaiYing 创建一套以 {主色与辅助色} 为主色调、以 {主题或人物} 为视觉中心的 AI 皮肤；画面呈现 {场景与氛围}，并适合 {使用场景或用途}。请据此生成协调的背景、徽记和界面配色，并自动应用到 BaiYing。',
      '把 BaiYing 变成蓝白冠军之夜：传奇 10 号在金色纸雨中高举世界冠军奖杯，热血、荣耀，像一件值得收藏的球迷纪念品',
      '把 BaiYing 打造成明亮通透的红金招财主题：一位威严亲和的东方财神作为主视觉，以大面积象牙白和暖米色为画布，朱红、金色、祥云与流动金线作为点缀，喜庆华贵但不俗艳，适合行情分析、投资研究与每日复盘',
      '设计一套红色灯海与东方舞台美学的应援外观：青年演员歌手的温柔剪影、银色追光和克制高级的纪念感',
      '把 BaiYing 打造成一间有橘猫陪伴的雨后清晨书房：奶油白、浅木色、柔和天光和窗边绿植，空气清新、安静治愈，整体明亮轻盈，适合阅读、学习与日常工作',
      '把 BaiYing 变成深蓝数据与自动化指挥舱：抽象趋势光轨、精密网格和琥珀色状态灯，适合代码、分析、盯盘与长期任务',
