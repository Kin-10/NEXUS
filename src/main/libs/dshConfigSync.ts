// Renders BaiYing provider configuration into the dsh settings file
// ($DSH_HOME/settings.yaml) so a kit-installed dsh boots with the user's
// models already available. Ownership rules:
//   - only providers keyed `baiying-<id>` are managed (rewritten each sync);
...
export const DSH_MANAGED_PROVIDER_PREFIX = 'baiying-';

// dsh groups its model picker by display name, so every synced route carries
// this marker: without it a BaiYing-managed provider is indistinguishable
// from one the user added inside dsh.
export const DSH_MANAGED_LABEL_PREFIX = 'BaiYing · ';
...
export const DSH_PLAN_ROUTE_ID = 'baiying-plan';
export const DSH_PLAN_ANTHROPIC_ROUTE_ID = 'baiying-plan-anthropic';
...
export function mapApiFormatToDshProtocol(apiFormat: string | undefined): DshProviderRoute['api'] | null {
  ...
  // BaiYing's OpenAI-format providers are overwhelmingly third-party
...
export function renderDshManagedSettings(
  ...
  for (const [providerId, config] of Object.entries(providers)) {
    ...
    routes[routeId] = {
      ...
      // ("deepseek"), and mark the entry as BaiYing-managed.
      displayName:
