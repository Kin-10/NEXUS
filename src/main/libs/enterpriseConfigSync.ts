 ...
 * (e.g., 'deepseek', 'anthropic') instead of the generic 'baiying'.
 */
function syncModelConfig(configPath: string, store: SqliteStore): void {
  ...
  try {
    ...
    for (const [providerId, providerConfig] of Object.entries(models.providers)) {
      ...
      // Resolve apiKey: use plain text value, skip placeholders like ${baiying_...}
...
function syncIMChannels(configPath: string, imStore: IMStore): void {
  ...
  try {
    ...
    const PLATFORM_SETTERS: Record<string, (cfg: any) => void> = {
      ...
      'baiying-bee': (cfg) => imStore.setbaiyingBeeChanConfig(cfg),
    };
...
function syncOpenClawAgentList(
  ...
  try {
    ...
    if (syncedCount > 0) {
      console.log(`[Enterprise] synced ${syncedCount} agent config(s) to baiying agents`);
    }
  } catch (error) {
