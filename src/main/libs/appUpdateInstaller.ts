...
export async function downloadUpdate(
  ...
  const ts = Date.now();
  const downloadPath = path.join(updateDir, `baiying-update-${source}-${ts}${ext}.download`);
  const finalPath = path.join(updateDir, `baiying-update-${source}-${ts}${ext}`);
...
const WINDOWS_INSTALLER_PATH_ENV = 'BAIYING_UPDATE_INSTALLER_PATH';
...
async function installWindowsNsis(
  ...
  // customCheckAppRunning macro stops remaining BaiYing processes by image
  // name and polls until they are gone before replacing files. The installer
  // process itself is named baiying-update-*, so it is not affected by that
  // kill.
