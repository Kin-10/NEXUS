...
describe('buildAgentEntry', () => {
  test('emits explicit model.primary for the main agent', () => {
    const result = buildAgentEntry({
      ...
      systemPrompt: '',
      identity: '',
      model: 'baiying-server/deepseek-v3.2',
      workingDirectory: '',
      icon: '',
      skillIds: [],
      enabled: true,
    ...
    expect(result).toMatchObject({
      id: 'main',
      model: { primary: 'baiying-server/deepseek-v3.2' },
    });
  ...
  test('keeps explicit server model.primary when a custom provider has the same model id', () => {
    const result = buildAgentEntry({
      ...
      systemPrompt: '',
      identity: '',
      model: 'baiying-server/kimi-k2.6',
      workingDirectory: '',
      icon: '',
      skillIds: [],
      enabled: true,
    ...
    expect(result).toMatchObject({
      id: 'main',
      model: { primary: 'baiying-server/kimi-k2.6' },
    });
  });
...
describe('parsePrimaryModelRef', () => {
  test('parses provider-qualified primary model refs', () => {
    expect(parsePrimaryModelRef('baiying-server/deepseek-v3.2')).toEqual({
      providerId: 'baiying-server',
      modelId: 'deepseek-v3.2',
      primaryModel: 'baiying-server/deepseek-v3.2',
    });
  });
...
describe('resolveManagedSessionModelTarget', () => {
  const availableProviders = {
    'baiying-server': { models: [{ id: 'qwen3.5-plus' }, { id: 'deepseek-v3.2' }] },
  ...
  test('uses fallback target when agent model is empty', () => {
    expect(resolveManagedSessionModelTarget({
      agentModel: '',
      fallbackPrimaryModel: 'baiying-server/qwen3.5-plus',
      availableProviders,
    })).toEqual({
      providerId: 'baiying-server',
      modelId: 'qwen3.5-plus',
      primaryModel: 'baiying-server/qwen3.5-plus',
    });
  });
  ...
  test('falls back to current provider when bare model cannot be resolved uniquely', () => {
    expect(resolveManagedSessionModelTarget({
      ...
      fallbackPrimaryModel: 'baiying-server/qwen3.5-plus',
      availableProviders,
      currentProviderId: 'baiying-server',
    })).toEqual({
      providerId: 'baiying-server',
      modelId: 'unknown-model',
      primaryModel: 'baiying-server/unknown-model',
    });
  });
});
...
describe('resolveQualifiedAgentModelRef', () => {
  test('qualifies bare model ids when exactly one provider matches', () => {
    expect(resolveQualifiedAgentModelRef({
      agentModel: 'deepseek-v3.2',
      availableProviders: {
        'baiying-server': { models: [{ id: 'deepseek-v3.2' }] },
        minimax: { models: [{ id: 'MiniMax-M2.7' }] },
      },
    })).toEqual({
      status: 'qualified',
  ...
  test('does not auto-qualify bare model ids when multiple providers match', () => {
    expect(resolveQualifiedAgentModelRef({
      ...
      status: 'ambiguous',
      modelId: 'deepseek-v3.2',
      providerIds: ['anthropic', 'baiying-server'],
    });
  });
  ...
  test('keeps explicit server refs when a custom provider has the same model id', () => {
    expect(resolveQualifiedAgentModelRef({
      agentModel: 'baiying-server/kimi-k2.6',
      availableProviders: {
        moonshot: { models: [{ id: 'kimi-k2.6' }] },
      },
    })).toEqual({
      status: 'qualified',
      primaryModel: 'baiying-server/kimi-k2.6',
    });
  });
});
