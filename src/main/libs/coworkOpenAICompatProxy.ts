...
function shouldRefreshProxyToken(status: number, provider?: string): boolean {
  ...
  return status === 403 && provider !== ProviderName.BaiyingServer;
}

function isTemporaryBaiYingAuthRefreshFailure(
  provider: string | undefined,
  result: AuthTokenRefreshResult,
): boolean {
  return provider === ProviderName.BaiyingServer
...
async function handleRequest(
      ...
      if (
        ...
        if (refresher) {
          ...
          try {
            ...
            } else if (isTemporaryBaiYingAuthRefreshFailure(
              upstreamConfig.provider,
              refreshResult,
            )) {
              writeJSON(
  ...
  // Inject session_id and user_message for baiying-server logging only.
  // Strict providers (e.g. Gemini) reject unknown payload fields.
  if (upstreamConfig.provider === 'baiying-server') {
...
export const __openAICompatProxyTestUtils = {
  ...
  isTemporaryBaiYingAuthRefreshFailure,
  isGeminiProvider,
