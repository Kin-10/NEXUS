...
describe('appendLoginParams', () => {
  test('appends params inside hash route query for portal URLs', () => {
    const result = appendLoginParams(
      'https://baiying.youdao.com/portal#/login',
      {
        source: 'electron',
    ...
    expect(result).toBe(
      'https://baiying.youdao.com/portal#/login?source=electron&redirect_uri=http%3A%2F%2F127.0.0.1%3A43210%2Fauth%2Fcallback&state=test-state',
    );
  });

  test('preserves existing hash route params', () => {
    const result = appendLoginParams(
      'https://baiying.youdao.com/portal#/login?invitationCode=ABC123',
      { source: 'electron' },
    );

    expect(result).toBe(
      'https://baiying.youdao.com/portal#/login?invitationCode=ABC123&source=electron',
    );
  });
...
describe('appendCallbackReturnTo', () => {
  test('adds portal return URL to the local callback redirect URI', () => {
    const result = appendCallbackReturnTo(
      ...
      'https://baiying.youdao.com/portal#/login?source=electron&electronLogin=success',
    );

    expect(result).toBe(
      'http://127.0.0.1:43210/auth/callback?return_to=https%3A%2F%2Fbaiying.youdao.com%2Fportal%23%2Flogin%3Fsource%3Delectron%26electronLogin%3Dsuccess',
    );
  });
});
