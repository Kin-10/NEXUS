...
/**
 * Resolve whether a run model reference belongs to baiying-server without
...
export function resolveServerModelRefForRun(options: {
  ...
  if (explicitTarget) {
    if (explicitTarget.providerId === OpenClawProviderId.BaiyingServer) {
      return {
  ...
  const matchingProviders = Object.entries(options.availableProviders)
    ...
    .map(([providerId]) => providerId);
  const serverMatched = matchingProviders.includes(OpenClawProviderId.BaiyingServer);

  if (serverMatched && matchingProviders.length === 1) {
    return {
      status: ServerModelRefResolutionStatus.Server,
      modelId: modelRef,
      primaryModel: `${OpenClawProviderId.BaiyingServer}/${modelRef}`,
    };
  }
  if (serverMatched) {
    return {
...
const RENAMED_PROVIDER_IDS: Record<string, string> = {
  'github-copilot': 'baiying-copilot',
};
...
export function migrateAgentModelRefs(options: {
  ...
  for (const agent of agents) {
    ...
    // provider IDs (e.g. 'github-copilot' → 'baiying-copilot') are corrected
