...
export function ensureElectronNodeShim(electronPath: string, npmBinDir?: string): string | null {
  try {
    ...
    const nodeShContent = [
      '#!/usr/bin/env bash',
      'if [ -z "${BAIYING_ELECTRON_PATH:-}" ]; then',
      '  echo "BAIYING_ELECTRON_PATH is not set" >&2',
      '  exit 127',
      'fi',
      'exec env ELECTRON_RUN_AS_NODE=1 "${BAIYING_ELECTRON_PATH}" "$@"',
      '',
    ].join('\n');
    ...
    if (process.platform === 'win32') {
      ...
      const nodeCmdContent = [
        '@echo off',
        'if "%BAIYING_ELECTRON_PATH%"=="" (',
        '  echo BAIYING_ELECTRON_PATH is not set 1>&2',
        '  exit /b 127',
        ')',
        'set ELECTRON_RUN_AS_NODE=1',
        '"%BAIYING_ELECTRON_PATH%" %*',
        '',
      ].join('\r\n');
    ...
    if (npmBinDir && existsSync(npmBinDir)) {
      ...
      if (existsSync(npxCliJs)) {
        ...
        // npx.cmd for Windows — uses %BAIYING_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          const npxCmd = join(shimDir, 'npx.cmd');
          const npxCmdContent = [
            '@echo off',
            '"%~dp0node.cmd" "%BAIYING_NPM_BIN_DIR%\\npx-cli.js" %*',
            '',
          ].join('\r\n');
          writeFileSync(npxCmd, npxCmdContent, 'utf8');
          coworkLog('INFO', 'resolveNodeShim', `Created npx.cmd shim: ${npxCmd} (using env var BAIYING_NPM_BIN_DIR)`);
        }
      } else {
      ...
      if (existsSync(npmCliJs)) {
        ...
        // npm.cmd for Windows — uses %BAIYING_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          ...
          coworkLog('INFO', 'resolveNodeShim', `Created npm.cmd shim: ${npmCmd} (using env var BAIYING_NPM_BIN_DIR)`);
        }
      } else {
...
function resolveWindowsGitBashPath(): string | null {
  ...
  // 2. Bundled PortableGit (preferred default in BaiYing package)
...
function ensureWindowsBashUtf8InitScript(): string | null {
  try {
    ...
    const content = [
      '#!/usr/bin/env bash',
      '# Auto-generated by BaiYing – switch Windows console code page to UTF-8',
...
function applyPackagedEnvOverrides(env: Record<string, string | undefined>): void {
  ...
  if (app.isPackaged && !env.BAIYING_ELECTRON_PATH) {
    env.BAIYING_ELECTRON_PATH = electronNodeRuntimePath;
  }
  ...
  if (process.platform === 'win32') {
    ...
    if (configuredBashPath && bashPath === configuredBashPath) {
      ...
      if (!configuredHealth.ok) {
        ...
        if (fallbackPath && fallbackPath !== configuredBashPath) {
          ...
          env.BAIYING_GIT_BASH_RESOLUTION_ERROR = diagnostic;
          coworkLog('WARN', 'resolveGitBash', diagnostic);
          bashPath = null;
        }
      }
    ...
    if (bashPath) {
      ...
      delete env.BAIYING_GIT_BASH_RESOLUTION_ERROR;
      ...
      env.BAIYING_GIT_BASH_RESOLUTION_ERROR = truncateDiagnostic(diagnostic);
    }
  ...
  env.BAIYING_NPM_BIN_DIR = npmBinDir;
  ...
  if (shouldInjectShim) {
    ...
    if (shimDir) {
      ...
      env.BAIYING_NODE_SHIM_ACTIVE = '1';
    ...
    delete env.BAIYING_NODE_SHIM_ACTIVE;
...
function verifyNodeEnvironment(env: Record<string, string | undefined>): void {
        ...
        if (tool === 'node' && resolvedForExec) {
          try {
            ...
            if (process.platform === 'win32' && /\.cmd$/i.test(resolvedForExec)) {
              execTarget = env.BAIYING_ELECTRON_PATH || process.execPath;
            }
  ...
  coworkLog('INFO', tag, `BAIYING_ELECTRON_PATH=${env.BAIYING_ELECTRON_PATH || '(not set)'}`);
  coworkLog('INFO', tag, `BAIYING_NPM_BIN_DIR=${env.BAIYING_NPM_BIN_DIR || '(not set)'}`);
...
export function getSkillsRoot(): string {
  ...
  const envRoots = [process.env.BAIYING_SKILLS_ROOT, process.env.SKILLS_ROOT]
...
function finalizeNodeToolEnv(env: Record<string, string | undefined>): void {
  if (process.platform === 'win32' || env.BAIYING_NODE_SHIM_ACTIVE === '1') {
    env.BAIYING_ELECTRON_PATH = getElectronNodeRuntimePath().replace(/\\/g, '/');
  } else {
    delete env.BAIYING_ELECTRON_PATH;
  }
}
...
export async function getEnhancedEnv(target: OpenAICompatProxyTarget = 'local'): Promise<Record<string, string | undefined>> {
  ...
  env.BAIYING_SKILLS_ROOT = skillsRoot; // Alternative name for clarity
  finalizeNodeToolEnv(env);
  await injectSystemProxyForSubprocess(env);
