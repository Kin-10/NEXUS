...
function tryBaiyingServerFallback(modelId?: string): MatchedProvider | null {
  ...
  const effectiveApiFormat = cachedMeta?.apiFormat
    ...
    : 'openai';
  console.debug('[ClaudeSettings] baiying-server provider resolved:', {
    baseURL,
...
function resolveMatchedProvider(appConfig: AppConfig): { matched: MatchedProvider | null; error?: string } {
  ...
  if (!modelId) {
    ...
    if (!fallback) {
      const serverFallback = tryBaiyingServerFallback(configuredModelId);
  ...
  // Handle baiying-server provider: dynamically construct from auth tokens
  if (preferredProviderName === ProviderName.BaiyingServer) {
    const serverMatch = tryBaiyingServerFallback(modelId);
    if (serverMatch) {
...
export function resolveCurrentApiConfig(target: OpenAICompatProxyTarget = 'local'): ApiConfigResolution {
  ...
  return {
    config: {
      apiKey: resolvedApiKey || 'baiying-openai-compat',
...
export function resolveAllProviderApiKeys(): Record<string, string> {
  ...
  // baiying-server token is now managed by the token proxy
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    ...
    result[envName] = apiKey || 'sk-baiying-local';
  }
...
export function resolveAllEnabledProviderConfigs(): ProviderRawConfig[] {
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    if (!providerConfig?.enabled) continue;
    if (providerName === ProviderName.BaiyingServer) continue;
    ...
    result.push({
      providerName,
      baseURL: effectiveBaseURL,
      apiKey: apiKey || 'sk-baiying-local',
      apiType: effectiveApiFormat === 'anthropic' ? 'anthropic' : 'openai',
      authType: providerConfig.authType,
      codingPlanEnabled: !!providerConfig.codingPlanEnabled,
      models,
