...
test('parseManagedSessionKey handles raw local session keys', () => {
  expect(parseManagedSessionKey('baiying:abc-123')).toEqual({
    agentId: null,
    sessionId: 'abc-123',
  });
});
...
test('parseManagedSessionKey handles canonical local session keys', () => {
  expect(parseManagedSessionKey('agent:main:baiying:abc-123')).toEqual({
    agentId: 'main',
    sessionId: 'abc-123',
  });
});
...
test('buildManagedSessionKey emits canonical local session keys', () => {
  ...
  ).toBe(`agent:${DEFAULT_MANAGED_AGENT_ID}:baiying:abc-123`);
  expect(
    buildManagedSessionKey('abc-123', 'secondary'),
  ).toBe('agent:secondary:baiying:abc-123');
});

test('parseChannelSessionKey ignores managed local session keys', () => {
  expect(parseChannelSessionKey('baiying:abc-123')).toBe(null);
  expect(parseChannelSessionKey('agent:main:baiying:abc-123')).toBe(null);
});

test('channel sync does not treat managed local session keys as channel sessions', () => {
  const sync = createSync();

  expect(isManagedSessionKey('agent:main:baiying:abc-123')).toBe(true);
  expect(sync.isChannelSessionKey('agent:main:baiying:abc-123')).toBe(false);
  expect(sync.resolveOrCreateSession('agent:main:baiying:abc-123')).toBe(null);
