...
describe('buildMediaGenerationTurnInstruction', () => {
  test('keeps an ordinary image request to exactly one generation call', () => {
    ...
    expect(instruction).not.toContain('baiying_skin_manage');
  });
  ...
  test('guides a two-slot serial skin flow without a hard generation quota', () => {
    ...
    expect(instruction).toContain('soft budget of about two serial baiying_image_generate calls');
    ...
    expect(instruction).not.toContain('you must call the baiying_image_generate tool exactly once');
  });

  test('uses the native image route for a skin workflow without BaiYing image selection', () => {
    const instruction = buildMediaGenerationTurnInstruction(
      undefined,
      false,
      SkinWorkflowKind.SkinPack,
    ...
    expect(instruction).toContain('There is no baiying_image_generate status step');
    expect(instruction).toContain('baiying_skin_manage');
    expect(instruction).not.toContain('BaiYing media generation tools - NOT AVAILABLE');
  });

  test('uses the BaiYing image route for auto media selection', () => {
    const instruction = buildMediaGenerationTurnInstruction(
    ...
    expect(instruction).toContain('locked to baiying_image_generate');
  ...
  test('preserves the media-skill fallback when BaiYing tools are unavailable', () => {
    const instruction = buildMediaGenerationTurnInstruction(undefined, true);

    expect(instruction).toContain('BaiYing media generation tools - NOT AVAILABLE');
