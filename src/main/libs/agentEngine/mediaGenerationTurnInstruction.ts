...
const buildSkinPackInstruction = (selection?: CoworkMediaSelection): string => {
  ...
  const lines = [
    ...
    'Use the current workflow draft when one already exists; otherwise call baiying_skin_manage with action="create_draft", include the validated immersive_shell presentation described by the bundled Skill, and retain the returned skinId for every later skin operation.',
  ...
  if (useLobsterImageTool) {
    ...
    if (imageModel) {
      ...
      lines.push('You may call baiying_image_generate action="list" once before generation to choose one image model, then lock that model for both assets. Listing is not an image generation attempt.');
    }
    lines.push('If a generate call returns an asynchronous taskId, call baiying_image_generate with action="status" exactly once. That single status call polls internally to a terminal state; never busy-poll or repeat it.');
  } else {
    lines.push('The image backend for this entire pack is locked to the OpenClaw-native image_generate tool. Do not call baiying_image_generate, seedream, seedance, or any other image tool or skill.');
    ...
    lines.push('After each image_generate action="generate" call, wait for that call or its completion event to reach terminal success. There is no baiying_image_generate status step for this route.');
  }
  ...
  lines.push('Do not start the next slot until the current generation reaches terminal status="succeeded" and baiying_skin_manage action="register_asset" succeeds for the current skinId, slot, and exact generated local sourcePath.');
  ...
  lines.push('After both assets are registered, call baiying_skin_manage action="status" with skinId to confirm the draft is ready, then call action="apply" with the same skinId. Starting this Kit is an explicit request to apply the completed skin.');
...
export const buildMediaGenerationTurnInstruction = (
  ...
  if (!selection || selection.mode === 'none') {
    if (hasMediaSkillActive) {
      return [
        '[BaiYing media generation tools - NOT AVAILABLE]',
        'The baiying_image_generate and baiying_video_generate tools are NOT available for this turn.',
        'Do NOT call baiying_image_generate or baiying_video_generate.',
  ...
  const lines = [
    '[BaiYing media generation turn instruction]',
    'The user selected a BaiYing media generation model for this turn.',
    'IMPORTANT: Do NOT read or use the "seedance" or "seedream" skills for this request.',
    'The BaiYing media generation tools (baiying_image_generate / baiying_video_generate) replace those skills when a media model is selected.',
    'Do not run any skill scripts for image or video generation. Use only the baiying_* tools specified below.',
  ];

  if (selection.mode === 'image') {
    lines.push('If the current user request asks to create, generate, draw, render, or make an image/photo/picture, you must call the baiying_image_generate tool exactly once with action="generate".');
    ...
    lines.push('If the current user request asks to create, generate, render, or make a video, you must call the baiying_video_generate tool exactly once with action="generate".');
    ...
    lines.push('If the current user request asks for image generation, call baiying_image_generate with action="generate".');
    lines.push('If the current user request asks for video generation, call baiying_video_generate with action="generate".');
  ...
  if (!selection.imageModelId && !selection.videoModelId && selection.modelId?.trim()) {
    lines.push(`You MUST use model "${selection.modelId.trim()}" for media generation. Do NOT use a different model unless the user explicitly requests a different BaiYing media model by name.`);
  }
