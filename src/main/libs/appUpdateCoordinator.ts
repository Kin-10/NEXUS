...
const APP_UPDATE_TEST_CURRENT_VERSION_ENV = 'BAIYING_UPDATE_CURRENT_VERSION';
...
export class AppUpdateCoordinator {
  ...
  private isCachedInstallerForSource(filename: string, source: AppUpdateSource | null): boolean {
    if (!filename.startsWith('baiying-update-')) {
      return false;
    }
    if (source == null) {
      return true;
    }
    if (filename.startsWith(`baiying-update-${source}-`)) {
      return true;
    }
    return /^baiying-update-\d+/.test(filename);
  }
