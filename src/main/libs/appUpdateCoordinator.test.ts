...
function seedReadyFile(store: SqliteStore, updatesDir: string, source: AppUpdateSource): string {
  ...
  const filePath = path.join(updatesDir, `baiying-update-${source}-1${extension}`);
  ...
  store.set(readyFileStoreKey(source), {
    ...
    info: {
      ...
      url: `https://updates.example.com/baiying-${READY_VERSION}${extension}`,
    },
...
describe('AppUpdateCoordinator', () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'baiying-update-test-'));
      ...
      json: async () => ({
        ...
        data: {
          value: {
            version: READY_VERSION,
            windowsX64: {
              url: 'http://downloads.example/BaiYing.exe',
            },
          },
        },
      }),
  ...
  test('accepts a changing HTTPS CDN without passing a fixed origin allowlist', async () => {
    Object.defineProperty(process, 'platform', { value: 'win32' });
    const installerUrl = `https://replacement-cdn.example.net/BaiYing-${READY_VERSION}.exe`;
    const downloadedFile = path.join(updatesDir, 'baiying-update-auto-1.exe');
    mocks.fetch.mockResolvedValue({
      ok: true,
      json: async () => ({
        code: 0,
  ...
  test('does not restore a persisted Windows installer from an untrusted source', async () => {
    ...
    stored.info.url = 'http://downloads.example/BaiYing.exe';
  ...
  test('does not restore a symlinked Windows installer from the update cache', async () => {
    ...
    const symlinkPath = path.join(updatesDir, 'baiying-update-auto-2.exe');
    ...
    store.set(readyFileStoreKey(AppUpdateSource.Auto), {
      ...
      info: {
        ...
        url: `https://updates.example.com/baiying-${READY_VERSION}.exe`,
      },
      windowsInstallerUrlPolicyReceipt: {
        policyVersion: WINDOWS_INSTALLER_URL_POLICY_VERSION,
        inputOrigin: 'https://updates.example.com',
  ...
  test('revalidates a ready installer source immediately before elevation', async () => {
    ...
    internal.state.info.url = 'http://downloads.example/BaiYing.exe';
      ...
      json: async () => ({
        ...
        data: {
          value: {
            ...
            macIntel: { url: `https://updates.example.com/baiying-${READY_VERSION}.dmg` },
            macArm: { url: `https://updates.example.com/baiying-${READY_VERSION}.dmg` },
            windowsX64: { url: `https://updates.example.com/baiying-${READY_VERSION}.exe` },
          },
        },
      }),
    });
