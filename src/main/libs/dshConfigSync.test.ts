...
describe('sanitizeDshRouteId', () => {
  test('lowercases, collapses separators, and prefixes', () => {
    expect(sanitizeDshRouteId('OpenAI')).toBe('baiying-openai');
    expect(sanitizeDshRouteId('My_Provider 1')).toBe('baiying-my-provider-1');
  });

  test('strips leading non-letters and never returns a bare prefix', () => {
    expect(sanitizeDshRouteId('123abc')).toBe('baiying-abc');
    expect(sanitizeDshRouteId('自定义')).toBe('baiying-provider');
  });
});

describe('deriveDshApiKeyEnvRef', () => {
  test('follows the dsh models UI convention', () => {
    expect(deriveDshApiKeyEnvRef('baiying-my-gateway')).toBe('BAIYING_MY_GATEWAY_API_KEY');
  });
});
...
describe('renderDshManagedSettings', () => {
  test('renders a full route with env-referenced credentials', () => {
    ...
    const route = managed.routes['baiying-my-gw'];
    expect(route).toBeDefined();
    expect(route.apiKeyEnv).toBe('BAIYING_MY_GW_API_KEY');
    expect(managed.envVars.BAIYING_MY_GW_API_KEY).toBe('sk-test');
  ...
  test('prefers the canonical provider label over the raw config key, marked as managed', () => {
    ...
    expect(managed.routes['baiying-deepseek'].displayName).toBe(`${DSH_MANAGED_LABEL_PREFIX}DeepSeek`);
  });
  ...
  test('an explicit displayName still wins over the canonical label', () => {
    ...
    expect(managed.routes['baiying-deepseek'].displayName).toBe(`${DSH_MANAGED_LABEL_PREFIX}My DeepSeek`);
  });
  ...
  test('falls back to the config key for unknown providers', () => {
    ...
    expect(managed.routes['baiying-my-gw'].displayName).toBe(`${DSH_MANAGED_LABEL_PREFIX}my-gw`);
  });
  ...
  test('declares image input only when the model supports it', () => {
    ...
    expect(managed.routes['baiying-gw'].models[0].input).toEqual(['text', 'image']);
  });
  ...
  test('resolves the preferred default when it survived rendering', () => {
    ...
    ).toEqual({ provider: 'baiying-gw', model: 'model-a' });
  });
  ...
  test('falls back to a rendered route when the preferred one was skipped', () => {
    ...
    expect(managed.defaultModel).toEqual({ provider: 'baiying-gw', model: 'model-a' });
  });
...
describe('plan provider (token proxy)', () => {
  ...
  test('marks the plan as BaiYing-managed in the picker', () => {
    const managed = renderDshManagedSettings({}, { planProvider: plan });
...
describe('mergeDshSettingsText', () => {
  ...
  test('creates the document from scratch and seeds the default model', () => {
    ...
    expect((doc['llm-pi-ai'].providers as Record<string, unknown>)['baiying-gw']).toBeDefined();
    expect(doc['agent-default-model']).toEqual({ provider: 'baiying-gw', model: 'model-a' });
  });
  ...
  test('preserves user routes and foreign namespaces, replaces stale managed routes', () => {
    const existing = yaml.dump({
      ...
      'llm-pi-ai': {
        providers: {
          ...
          'baiying-stale': { apiKeyEnv: 'STALE', api: 'openai-completions', baseURL: 'https://s.example', models: [{ id: 's' }] },
        },
      },
    ...
    expect(providers['baiying-stale']).toBeUndefined();
    expect(providers['baiying-gw']).toBeDefined();
  ...
  test('repairs a default pinned to a managed route that no longer exists', () => {
    const existing = yaml.dump({ 'agent-default-model': { provider: 'baiying-removed', model: 'gone' } });
    const doc = yaml.load(mergeDshSettingsText(existing, managed).text) as Record<string, unknown>;
    expect(doc['agent-default-model']).toEqual({ provider: 'baiying-gw', model: 'model-a' });
  });
