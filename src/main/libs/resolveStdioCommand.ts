 ...
 * the BAIYING_NPM_BIN_DIR path without resolving API config or proxy settings.
 */
...
export async function resolveStdioCommand(server: McpServerRecord): Promise<ResolvedStdioCommand> {
  ...
  // (PATH, APPDATA, TEMP, etc.) — our node shims in PATH need BAIYING_ELECTRON_PATH
  // and BAIYING_NPM_BIN_DIR which won't be inherited. Pre-resolving to absolute
    ...
    if (nodeCommandType) {
      ...
      if (systemNode) {
        ...
        const withElectronNodeEnv = (base: Record<string, string> | undefined): Record<string, string> => ({
          ...
          BAIYING_ELECTRON_PATH: electronNodeRuntimePath,
        });
