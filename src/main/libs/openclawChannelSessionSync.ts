 ...
 * to local Cowork sessions so that conversations are visible in the BaiYing UI.
 */
...
const BAIYING_SESSION_PREFIX = 'baiying:';
...
export function buildManagedSessionKey(
  ...
  return `agent:${normalizedAgentId}:baiying:${normalizedSessionId}`;
}
...
export function parseManagedSessionKey(
  ...
  if (raw.startsWith(BAIYING_SESSION_PREFIX)) {
    const sessionId = raw.slice(BAIYING_SESSION_PREFIX.length).trim();
  ...
  if (parts.length < 4 || parts[0] !== 'agent' || parts[2] !== 'baiying') {
    return null;
  }
...
export class OpenClawChannelSessionSync {
  ...
  resolveOrCreateSession(sessionKey: string): string | null {
    // 1. Skip BaiYing-originated sessions
    if (isManagedSessionKey(sessionKey)) {
      console.log('[ChannelSessionSync] skipped: BaiYing-originated session');
      return null;
    }
