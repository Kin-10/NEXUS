...
async function writeZip(entries: NodeServicePackageEntry[]): Promise<{
  ...
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'baiying-node-deploy-'));
...
export async function packageNodeServiceDeployment(
  ...
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'baiying-node-build-'));
