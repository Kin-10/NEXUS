...
const makeTempDir = (): string => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'baiying-data-migration-test-'));
  tempRoots.push(dir);
  return dir;
};
...
const writeManifestFixture = (userDataPath: string): void => {
  writeFile(
    path.join(userDataPath, '.baiying-migration.json'),
    JSON.stringify({
      format: 'baiying-data-migration',
      version: 1,
      archiveRoot: 'BaiYing',
      sqlite: { exists: true },
...
test('createMigrationArchive excludes cache and log data and writes a manifest', () => {
  ...
  const userData = path.join(root, 'BaiYing');
  ...
  writeFile(path.join(userData, 'backups', 'sqlite', 'snapshots', 'baiying-latest.sqlite'), 'old-snapshot');
  writeFile(path.join(userData, 'sqlite-backups', 'baiying-latest.sqlite'), 'legacy-snapshot');
  ...
  expect(entries).toContain('BaiYing/.baiying-migration.json');
  expect(entries).toContain(`BaiYing/${DB_FILENAME}`);
  expect(entries).toContain('BaiYing/openclaw/state/openclaw.json');
  expect(entries).toContain('BaiYing/SKILLs/demo/SKILL.md');
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(extractRoot, 'BaiYing', '.baiying-migration.json'), 'utf8'),
  ) as {
    format?: string;
    archiveRoot?: string;
  ...
  expect(manifest.format).toBe('baiying-data-migration');
  expect(manifest.archiveRoot).toBe('BaiYing');
...
test('createMigrationArchive replaces the live sqlite database with the snapshot', () => {
  ...
  const extractRoot = extractArchive(archivePath);
  expect(readSqliteString(path.join(extractRoot, 'BaiYing', DB_FILENAME), 'SELECT value FROM kv WHERE key = ?', ['auth_tokens']))
    .toContain('snapshot-refresh');
  expect(fs.existsSync(path.join(extractRoot, 'BaiYing', `${DB_FILENAME}-wal`))).toBe(false);
});
...
test('inspectMigrationArchive rejects legacy Windows PowerShell archive root', () => {
  const root = makeTempDir();
  const legacyRoot = path.join(root, 'AppData', 'Roaming', 'BaiYing');
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(/does not contain BaiYing user data/);
});

test('inspectMigrationArchive rejects archives without a migration manifest', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'BaiYing');
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(/missing \.baiying-migration\.json/);
});
...
test('inspectMigrationArchive rejects archives whose manifest does not match sqlite content', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', 'BaiYing');
  ...
  const extractRoot = extractArchive(archivePath);
  const manifestPath = path.join(extractRoot, 'BaiYing', '.baiying-migration.json');
...
test('performPendingDataMigrationRestoreSync creates rollback and restores backup data', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', 'BaiYing');
  const targetUserData = path.join(root, 'target', 'BaiYing');
...
test('performPendingDataMigrationRestoreSync replaces data in place and preserves runtime locks', () => {
  ...
  writeFile(path.join(sourceUserData, 'backups', 'sqlite', 'snapshots', 'baiying-latest.sqlite'), 'source-backup');
  writeFile(path.join(sourceUserData, 'sqlite-backups', 'baiying-latest.sqlite'), 'source-legacy-backup');
  ...
  writeFile(path.join(targetUserData, 'old-only.txt'), 'old');
  writeFile(path.join(targetUserData, 'backups', 'sqlite', 'snapshots', 'baiying-latest.sqlite'), 'target-backup');
  writeFile(path.join(targetUserData, 'sqlite-backups', 'baiying-latest.sqlite'), 'target-legacy-backup');
  ...
  createMigrationArchiveSync({ userDataPath: sourceUserData, outputPath: archivePath });
  const extractRoot = extractArchive(archivePath);
  writeFile(path.join(extractRoot, 'BaiYing', 'backups', 'sqlite', 'snapshots', 'baiying-latest.sqlite'), 'legacy-source-backup');
  writeFile(path.join(extractRoot, 'BaiYing', 'sqlite-backups', 'baiying-latest.sqlite'), 'legacy-source-legacy-backup');
  writeFile(path.join(extractRoot, 'BaiYing', 'cowork', 'bin', 'node.cmd'), 'legacy-source-shim');
  writeFile(path.join(extractRoot, 'BaiYing', 'install-timing.log'), 'legacy-source-install-log');
  writeFile(path.join(extractRoot, 'BaiYing', 'skill-migrate.log'), 'legacy-source-skill-migrate-log');
  writeFile(path.join(extractRoot, 'BaiYing', 'Dictionaries', 'source.bdic'), 'legacy-source-dictionary');
  writeFile(path.join(extractRoot, 'BaiYing', 'Local Storage', 'leveldb', 'source.log'), 'legacy-source-local-storage');
  writeFile(path.join(extractRoot, 'BaiYing', 'Preferences'), 'legacy-source-preferences');
  writeFile(path.join(extractRoot, 'BaiYing', 'Session Storage', 'leveldb', 'source.log'), 'legacy-source-session-storage');
  writeFile(path.join(extractRoot, 'BaiYing', 'openclaw', 'logs', 'gateway-2026-06-10.log'), 'legacy-source-gateway-log');
  writeFile(path.join(extractRoot, 'BaiYing', 'openclaw', 'mcp-packages', 'demo', 'node_modules', 'native.node'), 'legacy-source-native');
  writeFile(path.join(extractRoot, 'BaiYing', 'openclaw', 'state', 'logs', 'commands.log'), 'legacy-source-commands-log');
  writeFile(path.join(extractRoot, 'BaiYing', 'runtimes', 'python', 'python.exe'), 'legacy-source-runtime');
  tar.create({
    sync: true,
    gzip: true,
    file: archivePath,
  ...
  expect(fs.existsSync(path.join(targetUserData, `${DB_FILENAME}-shm`))).toBe(false);
  expect(fs.existsSync(path.join(targetUserData, 'old-only.txt'))).toBe(false);
  expect(fs.readFileSync(path.join(targetUserData, 'backups', 'sqlite', 'snapshots', 'baiying-latest.sqlite'), 'utf8'))
    .toBe('target-backup');
  expect(fs.readFileSync(path.join(targetUserData, 'sqlite-backups', 'baiying-latest.sqlite'), 'utf8'))
