...
// config change) is trusted to complete before BaiYing resumes managing the
// process itself.
...
export function buildOpenClawCompileCacheEnv(compileCacheDir: string): NodeJS.ProcessEnv {
  return {
    ...
    // The cache is already configured by BaiYing. Prevent the packaged
...
export class OpenClawEngineManager extends EventEmitter {
   ...
   * active BaiYing must not kill/respawn the process: the gateway is between
  ...
  private async doStartGateway(): Promise<OpenClawEngineStatus> {
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      BAIYING_SKILLS_ROOT: skillsRoot,
      ...
      // Disable Bonjour/mDNS LAN discovery advertising.  BaiYing is a
      ...
      BAIYING_ELECTRON_PATH: electronNodeRuntimePath.replace(/\\/g, '/'),
      BAIYING_OPENCLAW_ENTRY: openclawEntry.replace(/\\/g, '/'),
    ...
    // find the BaiYing-managed Python instead of the Windows Store stub.
  ...
  private ensureOpenClawWorkerShimsForBundle(runtimeRoot: string): void {
    try {
      ...
      if (result.protectedExisting.length > 0) {
        console.warn(
          `[OpenClaw] Skipped ${result.protectedExisting.length} worker shim(s) because existing files are not BaiYing shims.`,
        );
      }
    } catch (error) {
  ...
  private ensureBundledCliShims(): string | null {
    ...
    const shellWrapper = [
      '#!/usr/bin/env bash',
      'if [ -z "${BAIYING_OPENCLAW_ENTRY:-}" ]; then',
      '  echo "BAIYING_OPENCLAW_ENTRY is not set" >&2',
      '  exit 127',
      'fi',
      'if [ -n "${BAIYING_ELECTRON_PATH:-}" ]; then',
      '  exec env ELECTRON_RUN_AS_NODE=1 "${BAIYING_ELECTRON_PATH}" "${BAIYING_OPENCLAW_ENTRY}" "$@"',
      'fi',
      'if command -v node >/dev/null 2>&1; then',
      '  exec node "${BAIYING_OPENCLAW_ENTRY}" "$@"',
      'fi',
      'echo "Neither BAIYING_ELECTRON_PATH nor node is available for OpenClaw CLI." >&2',
      'exit 127',
      '',
    ].join('\n');
    const windowsWrapper = [
      '@echo off',
      'if "%BAIYING_OPENCLAW_ENTRY%"=="" (',
      '  echo BAIYING_OPENCLAW_ENTRY is not set 1>&2',
      '  exit /b 127',
      ')',
      'if not "%BAIYING_ELECTRON_PATH%"=="" (',
      '  set ELECTRON_RUN_AS_NODE=1',
      '  "%BAIYING_ELECTRON_PATH%" "%BAIYING_OPENCLAW_ENTRY%" %*',
      '  exit /b %ERRORLEVEL%',
      ')',
      'node "%BAIYING_OPENCLAW_ENTRY%" %*',
      '',
    ].join('\r\n');
