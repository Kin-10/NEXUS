...
function buildSkillEnv(): Record<string, string | undefined> {
  ...
  env.BAIYING_ELECTRON_PATH = getElectronNodeRuntimePath();
...
const downloadGithubArchive = async (
    ...
    archiveUrlCandidates.push(
      {
        ...
        headers: { 'User-Agent': 'BaiYing Skill Downloader' },
      },
      {
  ...
  archiveUrlCandidates.push({
    ...
    headers: {
      ...
      'User-Agent': 'BaiYing Skill Downloader',
...
export class SkillManager {
      ...
      if (fs.existsSync(localSource)) {
        ...
        if (stat.isFile()) {
          if (isZipFile(localSource)) {
            ...
            const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'baiying-skill-zip-'));
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'baiying-skill-npm-'));
        cleanupPath = tempRoot;
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'baiying-skill-clawhub-'));
        cleanupPath = tempRoot;
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'baiying-skill-'));
        cleanupPath = tempRoot;
  ...
  private async performUpgradeDownload(skillId: string, downloadUrl: string, existingSkillDir: string): Promise<{
    ...
    try {
      ...
      const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'baiying-skill-upgrade-'));
      cleanupPath = tempRoot;
